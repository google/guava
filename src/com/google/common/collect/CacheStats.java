 ...
 *
 * <ul>
 * <li>A cache lookup that encounters an existing cache entry will increment {@code hitCount}.</li>
 ...
 *     total creation time, in nanoseconds, to {@code totalCreateTime}.</li>
 * <li>Cache lookups that encounter a missing cache entry that is pending creation will await
 *     successful creation and then increment {@code missCount}.</li>
 * <li>No stats are modified when entry creation throws an exception, neither for the creating
 *     thread nor for waiting threads.</li>
 * <li>When an entry is evicted from the cache, {@code evictionCount} is incremented.</li>
 * <li>No stats are modified when a cache entry is invalidated or manually removed.</li>
...
public final class CacheStats {
   ...
   */
  public CacheStats(long hitCount, long missCount, long createCount, long evictionCount,
  ...
  public double hitRate() {
    long reqeustCount = requestCount();
    return (reqeustCount == 0) ? 1.0 : (double) hitCount / reqeustCount;
  }

  /**
   * Returns the number of times {@link Cache#get} has returned an uncached (newly computed) value,
   * or null. Multiple concurrent calls to {@link Cache#get} on an absent value can result in
   * multiple misses, all returning the results of a single computation.
   */
  ...
  public double missRate() {
    long reqeustCount = requestCount();
    return (reqeustCount == 0) ? 0.0 : (double) missCount / reqeustCount;
  }

  /**
   * Returns the number of times {@link Cache#get} has successfully computed a new value. This
   * differs from {@link #missCount} only in the face of concurrent calls to {@link Cache#get} on an
   * absent value, in which case multiple simultaneous misses will result in a single computation.
  ...
  /**
   * Returns the total number of nanoseconds the cache has spent computing new values. This can be
  ...
  /**
   * Returns the average time spent computing new values. This is defined as
  ...
  @Override
  public int hashCode() {
    return Objects.hashCode(hitCount, missCount, createCount, evictionCount, totalCreateTime);
  }
