...
public class ListSubListTester<E> extends AbstractListTester<E> {
  ...
  public void testSubList_startGreaterThanEnd() {
    try {
       ...
       * implementations throw IllegalArgumentException:
  ...
  public void testSubList_entireList() {
    assertEquals("subList(0, size) should be equal to the original list", getList(), getList()
  ...
  public void testSubList_originalListSetAffectsSubList() {
    ...
    assertEquals("A set() call to a list after a sublist has been created "
        + "should be reflected in the sublist", Collections.singletonList(e3()), subList);
  }
  ...
  public void testSubList_isEmpty() {
    List<E> list = getList();
    int size = getNumElements();
    for (List<E> subList : Arrays.asList(list.subList(0, size), list.subList(0, size - 1),
        list.subList(1, size), list.subList(0, 0), list.subList(size, size))) {
   ...
   * that tests of {@link CopyOnWriteArrayList} can suppress them with
   ...
   * {@link CopyOnWriteArrayList} can suppress them with
   * {@code FeatureSpecificTestSuiteBuilder.suppressing()} until <a
   * href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6570631">Sun bug 6570631</a> is fixed.
   */
  @GwtIncompatible("reflection")
  ...
  /**
   * Returns the {@link Method} instance for
   * {@link #testSubList_subListRemoveAffectsOriginalLargeList()} so that tests of
   * {@link CopyOnWriteArrayList} can suppress it with
   * {@code FeatureSpecificTestSuiteBuilder.suppressing()} until <a
