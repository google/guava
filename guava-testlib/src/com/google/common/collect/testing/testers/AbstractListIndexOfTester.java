...
public abstract class AbstractListIndexOfTester<E> extends AbstractListTester<E> {
  ...
  public void testFind_yes() {
    assertEquals(getMethodName() + "(firstElement) should return 0", 0, find(getOrderedElements()
        .get(0)));
  }
  ...
  public void testFind_nullContained() {
    initCollectionWithNullElement();
    assertEquals(getMethodName() + "(null) should return " + getNullLocation(), getNullLocation(),
        find(null));
  }

  public void testFind_wrongType() {
    try {
      assertEquals(getMethodName() + "(wrongType) should return -1 or throw", -1,
