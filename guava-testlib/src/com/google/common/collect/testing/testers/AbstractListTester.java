...
public class AbstractListTester<E> extends AbstractCollectionTester<E> {
  /*
   * Previously we had a field named list that was initialized to the value of collection in
   * setUp(), but that caused problems when a tester changed the value of list or collection but not
   * both.
   */
   ...
   * {@link AbstractCollectionTester#expectContents(Collection)} to verify that the order of the
   * elements in the list under test matches what is expected.
   */
  @Override
  ...
  private String reportContext(List<E> expected) {
    return Platform.format("expected collection %s; actual collection %s", expected,
        this.collection);
  }
}
