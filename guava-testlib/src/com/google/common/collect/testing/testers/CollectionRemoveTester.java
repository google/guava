...
public class CollectionRemoveTester<E> extends AbstractCollectionTester<E> {
  ...
  public void testRemove_present() {
    ...
    assertEquals("remove(present) should decrease a collection's size by one.", initialSize - 1,
        collection.size());
    expectMissing(e0());
  }
  ...
  public void testRemove_unsupportedNotPresent() {
    try {
      assertFalse("remove(notPresent) should return false or throw "
          + "UnsupportedOperationException", collection.remove(e3()));
  ...
  public void testRemove_nullNotSupported() {
    try {
      assertFalse("remove(null) should return false or throw " + "NullPointerException",
