...
public class MultisetRemoveTester<E> extends AbstractMultisetTester<E> {
  ...
  public void testRemove_occurrences_present() {
    assertEquals("multiset.remove(present, 2) didn't return the old count", 1, getMultiset()
        .remove(e0(), 2));
    assertFalse("multiset contains present after multiset.remove(present, 2)", getMultiset()
        .contains(e0()));
  ...
  public void testRemove_some_occurrences_present() {
    initThreeCopies();
    assertEquals("multiset.remove(present, 2) didn't return the old count", 3, getMultiset()
        .remove(e0(), 2));
    assertTrue("multiset contains present after multiset.remove(present, 2)", getMultiset()
        .contains(e0()));
  ...
  public void testRemove_occurrences_unsupported_absent() {
    ...
    try {
      assertEquals("multiset.remove(absent, 2) didn't return 0 or throw an exception", 0,
  ...
  public void testRemove_occurrences_0() {
    ...
    assertEquals("multiset.remove(E, 0) didn't return the old count", oldCount, getMultiset()
        .remove(e0(), 0));
  }
  ...
  public static List<Method> getRemoveDuplicateInitializingMethods() {
    return Arrays.asList(Helpers.getMethod(MultisetRemoveTester.class,
