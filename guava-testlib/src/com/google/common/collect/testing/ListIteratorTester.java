 ...
 * {@code elementsToInsert}, the elements to be passed to {@code set()} and {@code add()} calls;
 * {@code features}, the features supported by the iterator; and {@code expectedElements}, the
 * elements the iterator should return in order.
 * <p>
 * The items in {@code elementsToInsert} will be repeated if {@code steps} is larger than the number
...
public abstract class ListIteratorTester<E> extends AbstractIteratorTester<E, ListIterator<E>> {
  protected ListIteratorTester(int steps, Iterable<E> elementsToInsert,
      Iterable<? extends IteratorFeature> features, Iterable<E> expectedElements, int startIndex) {
