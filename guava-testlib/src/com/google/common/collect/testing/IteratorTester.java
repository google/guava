...
/**
 * A utility for testing an Iterator implementation by comparing its behavior to that of a
 * "known good" reference implementation. In order to accomplish this, it's important to test a
 * great variety of sequences of the {@link Iterator#next}, {@link Iterator#hasNext} and
 * {@link Iterator#remove} operations. This utility takes the brute-force approach of trying
 * <i>all</i> possible sequences of these operations, up to a given number of steps. So, if the
 * caller specifies to use <i>n</i> steps, a total of <i>3^n</i> tests are actually performed.
 *
 * <p>
 * For instance, if <i>steps</i> is 5, one example sequence that will be tested is:
 *
 * <ol>
 * <li>remove();
 * <li>hasNext()
 ...
 * {@link IteratorTester#newTargetIterator()} method. This is because it's impossible to test an
 * Iterator without changing its state, so the tester needs a steady supply of fresh Iterators.
 *
 * <p>
...
public abstract class IteratorTester<E> extends AbstractIteratorTester<E, Iterator<E>> {
   ...
   */
  protected IteratorTester(int steps, Iterable<? extends IteratorFeature> features,
