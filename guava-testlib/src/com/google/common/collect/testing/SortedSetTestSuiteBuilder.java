...
public class SortedSetTestSuiteBuilder<E> extends SetTestSuiteBuilder<E> {
   ...
   *
   * <p>
   * Because the set may be ascending or descending, this test must derive the relative order of
  ...
  final TestSuite createSubsetSuite(
    ...
    return newBuilderUsing(delegate, to, from)
        .named(parentBuilder.getName() + " subSet " + from + "-" + to).withFeatures(features)
        .suppressing(parentBuilder.getSuppressedTests()).createTestSuite();
  }

  /** Like using() but overrideable by NavigableSetTestSuiteBuilder. */
  SortedSetTestSuiteBuilder<E> newBuilderUsing(TestSortedSetGenerator<E> delegate, Bound to,
      Bound from) {
