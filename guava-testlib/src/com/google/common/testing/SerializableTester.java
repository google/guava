 ...
 *
 * <p>
 * <b>GWT warning:</b> Under GWT, both methods simply returns their input, as proper GWT
...
public final class SerializableTester {
   ...
   *
   * <p>
   * <b>GWT warning:</b> Under GWT, this method simply returns its input, as proper GWT
   * serialization tests require more setup. This no-op behavior allows test authors to intersperse
   * {@code SerializableTester} calls with other, GWT-compatible tests.
   *
   * <p>
   * Note that the specified object may not be known by the compiler to be a
   * {@link java.io.Serializable} instance, and is thus declared an {@code Object}. For example, it
   * might be declared as a {@code List}.
   *
   ...
   * those sublists might implement a private {@code MySubList} type but serialize as a plain
   * {@code MyList} to save space. So long as {@code MyList} has all the public supertypes of
   * {@code MySubList}, this is safe. For these cases, for which {@code reserializeAndAssert} is too
