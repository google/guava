 ...
 * extends {@code AbstractPackageSanityTests}. Currently sanity checks include
 * {@link NullPointerTester}, {@link EqualsTester} and {@link SerializableTester}. For example:
 * 
 * <pre>
 ...
 * static factory method. Neither are semantics of mutation methods such as
 * {@code someList.add(obj)} checked. For more detailed discussion of supported and unsupported
 * cases, see {@link #testEquals}, {@link #testNulls} and {@link #testSerializable}.
 *
 * <p>
 ...
 *
 * <p>
 * If not all classes on the classpath should be covered, {@link #ignoreClasses} can be used to
 * exclude certain classes. As a special case, classes with an underscore in the name (like
 * {@code AutoValue_Foo}) can be excluded using
...
public abstract class AbstractPackageSanityTests extends TestCase {
  ...
  private static final ImmutableList<String> NULL_TEST_METHOD_NAMES = ImmutableList.of("testNulls",
      "testNull", "testNullPointers", "testNullPointer", "testNullPointerExceptions",
  ...
  private static final ImmutableList<String> SERIALIZABLE_TEST_METHOD_NAMES = ImmutableList.of(
      "testSerializable", "testSerialization", "testEqualsAndSerializable",
  ...
  private static final ImmutableList<String> EQUALS_TEST_METHOD_NAMES = ImmutableList.of(
      "testEquals", "testEqualsAndHashCode", "testEqualsAndSerializable",
      "testEqualsAndSerialization", "testEquality");

  private static final Chopper TEST_SUFFIX = suffix("Test").or(suffix("Tests"))
      .or(suffix("TestCase")).or(suffix("TestSuite"));
   ...
   * explicit {@code testSerializable()} test in the corresponding {@code CTest} class, and
   * {@code C} will be excluded from automated serialization test performed by this method.
   */
  @Test
  public void testSerializable() throws Exception {
    // TODO: when we use @BeforeClass, we can pay the cost of class path scanning only once.
    for (Class<?> classToTest : findClassesToTest(loadClassesInPackage(),
   ...
   *
   * <p>
   * In all cases, if {@code C} needs custom logic for testing nulls, you can add an explicit
   ...
   * to construct the sample instances. In case of tie, the candidate constructors or factories are
  ...
  private static AssertionFailedError sanityError(Class<?> cls, List<String> explicitTestNames,
      String description, Throwable e) {
    String message =
        String.format(Locale.ROOT, "Error in automated %s of %s\n"
            + "If the class is better tested explicitly, you can add %s() to %sTest", description,
            cls, explicitTestNames.get(0), cls.getName());
