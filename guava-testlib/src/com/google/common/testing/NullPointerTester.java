 ...
 * {@link NullPointerException} or {@link UnsupportedOperationException} whenever null is passed to
 * a parameter that isn't annotated with {@link Nullable}.
 *
 * <p>
 ...
 * {@code NullPointerTester} uses best effort to pick non-null default values for many common JDK
 * and Guava types, and also for interfaces and public classes that have public parameter-less
 * constructors. When the non-null default value for a particular parameter type cannot be provided
 * by {@code NullPointerTester}, the caller can provide a custom non-null default value for the
 * parameter type via {@link #setDefault}.
 *
...
public final class NullPointerTester {
  ...
  /**
   * Runs {@link #testConstructor} on every constructor in class {@code c} that has at least
   * {@code minimalVisibility}.
   */
  ...
  /**
   * Runs {@link #testMethod} on every static method of class {@code c} that has at least
   ...
   * {@link UnsupportedOperationException} whenever <i>any</i> of its non-{@link Nullable}
   * parameters are null.
   *
  ...
  /**
   * Verifies that {@code method} produces a {@link NullPointerException} or
   * {@link UnsupportedOperationException} when the parameter in position {@code paramIndex} is
   * null. If this parameter is marked {@link Nullable}, this method does nothing.
   *
   * @param instance the instance to invoke {@code method} on, or null if {@code method} is static
   */
  public void testMethodParameter(@Nullable final Object instance, final Method method,
      int paramIndex) {
   ...
   *        static
   */
  private void testParameter(Object instance, Invokable<?, ?> invokable, int paramIndex,
      Class<?> testedClass) {
    ...
    try {
      ...
      AssertionFailedError error =
          new AssertionFailedError("wrong exception thrown from " + invokable + ": " + cause);
  ...
  private Object[] buildParamList(Invokable<?, ?> invokable, int indexOfParamToSetToNull) {
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if (i != indexOfParamToSetToNull) {
        ...
        Assert.assertTrue("Can't find or create a sample instance for type '" + param.getType()
            + "'; please provide one using NullPointerTester.setDefault()", args[i] != null
            || isNullable(param));
      }
    }
    return args;
