 ...
 *
 * <p>
 * Use this class to test code triggered by <em>finalization</em>, that is, one of the following
 ...
 *
 * <p>
 * Failure of the expected event to occur within an implementation-defined "reasonable" time period
 * or an interrupt while waiting for the expected event will result in a {@link RuntimeException}.
 *
 * <p>
 ...
 *
 * <p>
 * Even if your non-test code does not use finalization, you can use this class to test for leaks,
 * by ensuring that objects are no longer strongly referenced:
 *
 * <pre>
 * {@code
 ...
 *
 * <p>
 * This class only provides testing utilities. It is not designed for direct use in production or
 * for benchmarking.
 *
...
public final class GcFinalization {
  ...
  public static void await(CountDownLatch latch) {
    ...
    throw formatRuntimeException("Latch failed to count down within %d second timeout",
        timeoutSeconds);
  }
  ...
  public static void awaitDone(FinalizationPredicate predicate) {
    ...
    throw formatRuntimeException("Predicate did not become true within %d second timeout",
        timeoutSeconds);
  }
   ...
   * processing and may run concurrently, for example, if the JVM flag
   * {@code -XX:+ExplicitGCInvokesConcurrent} is used.
   *
   * <p>
   * Whenever possible, it is preferable to test directly for some observable change resulting from
   * GC, as with {@link #awaitClear}. Because there are no guarantees for the order of GC
   * finalization processing, there may still be some unfinished work for the GC to do after this
   * method returns.
   *
   * <p>
   * This method does not create any memory pressure as would be required to cause soft references
   * to be processed.
   *
   * @throws RuntimeException if timed out or interrupted while waiting
   * @since 12.0
