...
public final class TestingExecutors {
   ...
   * {@code execute/submit/schedule}, as in {@link CallerRunsPolicy}. This applies both to
   * individually submitted tasks and to collections of tasks submitted via {@code invokeAll},
   * {@code invokeAny}, {@code schedule}, {@code scheduleAtFixedRate}, and
   * {@code scheduleWithFixedDelay}. In the case of tasks submitted by {@code invokeAll} or
   * {@code invokeAny}, tasks will run serially on the calling thread. Tasks are run to completion
   ...
   * {@link MoreExecutors#newDirectExecutorService} and subject to the same constraints.
   *
   * <p>
   ...
   *
   * <p>
   * Because of the nature of single-thread execution, the methods {@code scheduleAtFixedRate} and
   * {@code scheduleWithFixedDelay} are not supported by this class and will throw an
  ...
  private static final class NoOpScheduledExecutorService extends AbstractListeningExecutorService
    ...
    private static class NeverScheduledFuture<V> extends AbstractFuture<V> implements
