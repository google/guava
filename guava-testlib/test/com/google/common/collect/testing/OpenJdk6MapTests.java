 ...
 */
/*
 * TODO(cpovirk): consider renaming this class in light of our now running it under JDK7
 */
public class OpenJdk6MapTests extends TestsForMapsInJavaUtil {
  ...
  protected Collection<Method> suppressForTreeMapNatural() {
    return Arrays.asList(getPutNullKeyUnsupportedMethod(), getPutAllNullKeyUnsupportedMethod(),
        getCreateWithNullKeyUnsupportedMethod(), getCreateWithNullUnsupportedMethod(), // for keySet
        getContainsEntryWithIncomparableKeyMethod(), getContainsEntryWithIncomparableValueMethod());
  }

  @Override
  protected Collection<Method> suppressForConcurrentHashMap() {
    /*
     * The entrySet() of ConcurrentHashMap, unlike that of other Map implementations, supports add()
     * under JDK8. This seems problematic, but I didn't see that discussed in the review, which
     * included many other changes: http://goo.gl/okTTdr
     * 
     * TODO(cpovirk): decide what the best long-term action here is: force users to suppress (as we
     * do now), stop testing entrySet().add() at all, make entrySet().add() tests tolerant of either
     * behavior, introduce a map feature for entrySet() that supports add(), or something else
     */
    return Arrays.asList(getAddUnsupportedNotPresentMethod(),
        getAddAllUnsupportedNonePresentMethod(), getAddAllUnsupportedSomePresentMethod());
  }

  @Override
  protected Collection<Method> suppressForConcurrentSkipListMap() {
    return Arrays.asList(getContainsEntryWithIncomparableKeyMethod(),
