...
public class FeatureEnumTest extends TestCase {
  private static void assertGoodTesterAnnotation(Class<? extends Annotation> annotationClass) {
    ...
    assertNotNull(rootLocaleFormat("%s must have a @Retention annotation.", annotationClass),
        retentionPolicy);
    assertEquals(rootLocaleFormat("%s must have RUNTIME RetentionPolicy.", annotationClass),
        RetentionPolicy.RUNTIME, retentionPolicy.value());
    assertNotNull(rootLocaleFormat("%s must be inherited.", annotationClass),
    ...
    for (String propertyName : new String[] {"value", "absent"}) {
      ...
      assertTrue(rootLocaleFormat("%s.%s() must return an array.", annotationClass, propertyName),
          returnType.isArray());
      assertSame(
          rootLocaleFormat("%s.%s() must return an array of %s.", annotationClass, propertyName,
              annotationClass.getDeclaringClass()), annotationClass.getDeclaringClass(),
  ...
  public static <E extends Enum<?> & Feature<?>> void assertGoodFeatureEnum(
    ...
    fail(rootLocaleFormat("Feature enum %s should contain an " + "annotation named 'Require'.",
        featureEnumClass));
  }
