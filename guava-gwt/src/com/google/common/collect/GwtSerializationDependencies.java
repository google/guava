 ...
 *
 * <p>
 * Because of our use of final fields in our collections, GWT's normal heuristic for determining
 ...
 *
 * <p>
 * For classes with final fields, GWT assumes that the class itself might be serialized but doesn't
 * assume the same about its final fields. To work around this, we provide dummy implementations of
 * our collections with their dependencies as non-final fields. Even though these implementations
 * are never instantiated, they are visible to GWT when it performs its serialization analysis, and
 * it assumes that their fields may be serialized.
 *
 * <p>
 * Currently we provide dummy implementations of all the immutable collection classes necessary to
 * support declarations like {@code ImmutableMultiset<String>} in RPC interfaces. Support for
 ...
 * RPC interface in terms of {@code HashMultiset}. It is still possible to send a
 * {@code HashMultiset} over GWT RPC; it is only the declaration of an interface in terms of
 * {@code HashMultiset} that we haven't tried to support. (We may wish to revisit this decision in
 * the future.)
 *
 * @author Chris Povirk
 */
...
final class GwtSerializationDependencies {
  ...
  /*
   * We support an interface declared in terms of LinkedListMultimap because it supports entry
   * ordering not supported by other implementations.
   */
  ...
  /*
   * We don't normally need "implements Serializable," but we do here. That's because ImmutableTable
   * itself is not Serializable as of this writing. We need for GWT to believe that this dummy class
   * is serializable, or else it won't generate serialization code for R, C, and V.
   */
