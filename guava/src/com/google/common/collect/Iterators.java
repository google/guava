...
public final class Iterators {
   ...
   *
   * @deprecated Use {@code ImmutableSet.<T>of().iterator()} instead; or for Java 7 or later,
   *             {@link Collections#emptyIterator}. This method is scheduled for removal in May
   *             2016.
   */
  @Deprecated
  ...
  public static String toString(Iterator<?> iterator) {
    return Collections2.STANDARD_JOINER.appendTo(new StringBuilder().append('['), iterator)
        .append(']').toString();
  }
   ...
   *
   * <p>
   * The returned iterator supports {@code remove()} when the corresponding input iterator supports
   * it.
   *
   * <p>
   * <b>Note:</b> the current implementation is not suitable for nested concatenated iterators, i.e.
   * the following should be avoided when in a loop:
   * {@code iterator = Iterators.concat(iterator, suffix);}, since iteration over the resulting
   * iterator has a cubic complexity to the depth of the nesting.
   */
   ...
   * an outer iterator containing two inner lists of three elements each, all in the original order.
   *
   * <p>
   * The returned lists implement {@link java.util.RandomAccess}.
   *
  ...
  /**
   * Returns all instances of class {@code type} in {@code unfiltered}. The returned iterator has
   * elements whose class is {@code type} or a subclass of {@code type}.
   *
   * @param unfiltered an iterator containing objects of any type
   * @param type the type of elements desired
   * @return an unmodifiable iterator containing all elements of the original iterator that were of
   *         the requested type
   */
  @SuppressWarnings("unchecked")
   ...
   * possible that <i>no</i> element will match, use {@link #tryFind} or
   ...
   * given predicate, if such an element exists. If no such element is found, an empty
   * {@link Optional} will be returned from this method and the iterator will be left exhausted: its
   * {@code hasNext()} method will return {@code false}.
   *
   * <p>
   * <b>Warning:</b> avoid using a {@code predicate} that matches {@code null}. If {@code null} is
   * matched in {@code iterator}, a NullPointerException will be thrown.
   *
   * @since 11.0
   */
   ...
   *
   * <p>
   * The returned iterator supports {@code remove()} if the provided iterator does. After a
  ...
  /**
   * Creates an iterator returning the first {@code limitSize} elements of the given iterator. If
   * the original iterator does not contain that many elements, the returned iterator will have the
   * same behavior as the original iterator. The returned iterator supports {@code remove()} if the
   * original iterator does.
   *
   ...
   *
   * <p>
   * The provided iterator must support {@link Iterator#remove()} or else the returned iterator will
   * fail on the first call to {@code next}.
   *
   ...
   *
   * <p>
   * <b>Note:</b> It is often preferable to represent your data using a collection type, for example
   * using {@link Arrays#asList(Object[])}, making this method unnecessary.
   *
   * <p>
   ...
   * {@code Arrays.asList(array).subList(offset, offset + length).listIterator(index)}.
   */
  static <T> UnmodifiableListIterator<T> forArray(final T[] array, final int offset, int length,
    ...
    /*
     * We can't use call the two-arg constructor with arguments (offset, end) because the returned
     * Iterator is a ListIterator that may be moved back past the beginning of the iteration.
     */
   ...
   *
   * <p>
   * This method has no equivalent in {@link Iterables} because viewing an {@code Enumeration} as an
   * {@code Iterable} is impossible. However, the contents can be <i>copied</i> into a collection
   ...
   *
   * <p>
   * Calls to the {@code peek} method with no intervening calls to {@code next} do not affect the
   ...
   * to the caller, although this is neither guaranteed to occur nor required to be consistent. For
   * example, this method <i>might</i> choose to pass through recognized implementations of
