...
public class TreeRangeSet<C extends Comparable<?>> extends AbstractRangeSet<C> {
  ...
  private Range<C> rangeEnclosing(Range<C> range) {
    checkNotNull(range);
    Entry<Cut<C>, Range<C>> floorEntry = rangesByLowerBound.floorEntry(range.lowerBound);
    return (floorEntry != null && floorEntry.getValue().encloses(range)) ? floorEntry.getValue()
        : null;
  }
    ...
    if (entryBelowLB != null) {
      ...
      if (rangeBelowLB.upperBound.compareTo(rangeToRemove.lowerBound) >= 0) {
        ...
        if (rangeToRemove.hasUpperBound()
          ...
          // { < > }
          replaceRangeWithSameLowerBound(Range.create(rangeToRemove.upperBound,
              rangeBelowLB.upperBound));
        }
        replaceRangeWithSameLowerBound(Range.create(rangeBelowLB.lowerBound,
  ...
  @VisibleForTesting
  static final class RangesByUpperBound<C extends Comparable<?>> extends
      AbstractNavigableMap<Cut<C>, Range<C>> {
    ...
    private RangesByUpperBound(NavigableMap<Cut<C>, Range<C>> rangesByLowerBound,
    ...
    @Override
    public NavigableMap<Cut<C>, Range<C>> subMap(Cut<C> fromKey, boolean fromInclusive,
        Cut<C> toKey, boolean toInclusive) {
    ...
    Iterator<Entry<Cut<C>, Range<C>>> entryIterator() {
      ...
      if (!upperBoundWindow.hasLowerBound()) {
        ...
        if (lowerEntry == null) {
          ...
          backingItr =
              rangesByLowerBound.tailMap(upperBoundWindow.lowerEndpoint(), true).values()
                  .iterator();
        }
      }
    ...
    Iterator<Entry<Cut<C>, Range<C>>> descendingEntryIterator() {
      ...
      if (upperBoundWindow.hasUpperBound()) {
        candidates =
            rangesByLowerBound.headMap(upperBoundWindow.upperEndpoint(), false).descendingMap()
                .values();
      } else {
      ...
      return new AbstractIterator<Entry<Cut<C>, Range<C>>>() {
        ...
        protected Entry<Cut<C>, Range<C>> computeNext() {
          ...
          Range<C> range = backingItr.next();
          return upperBoundWindow.lowerBound.isLessThan(range.upperBound) ? Maps.immutableEntry(
              range.upperBound, range) : endOfData();
        }
      };
    }
    ...
    public boolean isEmpty() {
      return upperBoundWindow.equals(Range.all()) ? rangesByLowerBound.isEmpty() : !entryIterator()
          .hasNext();
    }
  }

  private static final class ComplementRangesByLowerBound<C extends Comparable<?>> extends
      AbstractNavigableMap<Cut<C>, Range<C>> {
    ...
    Iterator<Entry<Cut<C>, Range<C>>> descendingEntryIterator() {
      ...
      if (positiveItr.hasNext()) {
        cut =
            (positiveItr.peek().upperBound == Cut.<C>aboveAll()) ? positiveItr.next().lowerBound
  ...
  private static final class SubRangeSetRangesByLowerBound<C extends Comparable<?>> extends
      AbstractNavigableMap<Cut<C>, Range<C>> {
    /**
    ...
    private NavigableMap<Cut<C>, Range<C>> subMap(Range<Cut<C>> window) {
      if (!window.isConnected(lowerBoundWindow)) {
        ...
        return new SubRangeSetRangesByLowerBound<C>(lowerBoundWindow.intersection(window),
            restriction, rangesByLowerBound);
      }
    }
    ...
    Iterator<Entry<Cut<C>, Range<C>>> entryIterator() {
      ...
      final Cut<Cut<C>> upperBoundOnLowerBounds =
          Ordering.natural().min(lowerBoundWindow.upperBound,
      ...
      final Iterator<Range<C>> completeRangeItr =
          rangesByLowerBound
              .headMap(upperBoundOnLowerBounds.endpoint(),
                  upperBoundOnLowerBounds.typeAsUpperBound() == BoundType.CLOSED).descendingMap()
              .values().iterator();
      return new AbstractIterator<Entry<Cut<C>, Range<C>>>() {
        @Override
        protected Entry<Cut<C>, Range<C>> computeNext() {
