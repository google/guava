...
/**
 * An immutable {@code SortedSet} that stores its elements in a sorted array. Some instances are
 * ordered by an explicit comparator, while others follow the natural sort ordering of their
 * elements. Either way, null elements are not supported.
 *
 * <p>
 * Unlike {@link Collections#unmodifiableSortedSet}, which is a <i>view</i> of a separate collection
 * that can still change, an instance of {@code ImmutableSortedSet} contains its own private data
 * and will <i>never</i> change. This class is convenient for {@code public static final} sets
 * ("constant sets") and also lets you easily make a "defensive copy" of a set provided to your
 * class by a caller.
 *
 * <p>
 * The sets returned by the {@link #headSet}, {@link #tailSet}, and {@link #subSet} methods share
 * the same array as the original set, preventing that array from being garbage collected. If this
 * is a concern, the data may be copied into a correctly-sized array by calling
 * {@link #copyOfSorted}.
 *
 * <p>
 ...
 * {@code ImmutableSortedSet} doesn't use {@link Object#equals} to determine if two elements are
 * equivalent. Instead, with an explicit comparator, the following relation determines whether
 ...
 * </pre>
 *
 * <b>Warning:</b> Like most sets, an {@code ImmutableSortedSet} will not function correctly if an
 * element is modified after being placed in the set. For this reason, and to avoid general
 * confusion, it is strongly recommended to place only immutable objects into this collection.
 *
 * <p>
 * <b>Note:</b> Although this class is not final, it cannot be subclassed as it has no public or
...
public abstract class ImmutableSortedSet<E> extends ImmutableSortedSetFauxverideShim<E> implements
  ...
  /**
   * Returns an immutable sorted set containing the given elements sorted by the given
   * {@code Comparator}. When multiple elements are equivalent according to {@code compare()}, only
   * the first one specified is included. This method iterates over {@code elements} at most once.
   *
   * <p>
   * Despite the method name, this method attempts to avoid actually copying the data when it is
   * safe to do so. The exact circumstances under which a copy will or will not be performed are
   ...
   * elements are in the first {@code k} positions of {@code contents}, and
  ...
  static <E> ImmutableSortedSet<E> construct(Comparator<? super E> comparator, int n, E... contents) {
    ...
    return new RegularImmutableSortedSet<E>(ImmutableList.<E>asImmutableList(contents, uniques),
        comparator);
  }
   ...
   * {@code toElement}, instead of throwing an exception, if passed a {@code toElement} greater than
   * an earlier {@code toElement}.
   */
  @Override
  ...
  /*
   * This class is used to serialize all ImmutableSortedSet instances, regardless of implementation
   * type. It captures their "logical contents" only. This is necessary to ensure that the existence
