...
/**
 * A high-performance, immutable {@code Set} with reliable, user-specified iteration order. Does not
 * permit null elements.
 *
 * <p>
 * Unlike {@link Collections#unmodifiableSet}, which is a <i>view</i> of a separate collection that
 * can still change, an instance of this class contains its own private data and will <i>never</i>
 * change. This class is convenient for {@code public static final} sets ("constant sets") and also
 * lets you easily make a "defensive copy" of a set provided to your class by a caller.
 *
 * <p>
 * <b>Warning:</b> Like most sets, an {@code ImmutableSet} will not function correctly if an element
 * is modified after being placed in the set. For this reason, and to avoid general confusion, it is
 * strongly recommended to place only immutable objects into this collection.
 *
 * <p>
 * This class has been observed to perform significantly better than {@link HashSet} for objects
 * with very fast {@link Object#hashCode} implementations (as a well-behaved immutable object
 * should). While this class's factory methods create hash-based instances, the
 * {@link ImmutableSortedSet} subclass performs binary searches instead.
 *
 * <p>
 * <b>Note:</b> Although this class is not final, it cannot be subclassed outside its package as it
 * has no public or protected constructors. Thus, instances of this type are guaranteed to be
 * immutable.
 *
 * <p>
 * See the Guava User Guide article on <a href=
 * "http://code.google.com/p/guava-libraries/wiki/ImmutableCollectionsExplained"> immutable
 * collections</a>.
 *
 * @see ImmutableList
 * @see ImmutableMap
...
public abstract class ImmutableSet<E> extends ImmutableCollection<E> implements Set<E> {
   ...
   * {@link Collections#emptySet}, and is preferable mainly for consistency and maintainability of
   * your code.
   */
  ...
  /**
   * Returns an immutable set containing a single element. This set behaves and performs comparably
   * to {@link Collections#singleton}, but will not accept a null element. It is preferable mainly
   * for consistency and maintainability of your code.
   */
  ...
  /**
   * Returns an immutable set containing the given elements, in order. Repeated occurrences of an
   * element (according to {@link Object#equals}) after the first are ignored.
   *
   ...
   * {@code k} is the size of the returned {@code ImmutableSet}, then the unique elements of
  ...
  /**
   * Returns an immutable set containing the given elements, in order. Repeated occurrences of an
   * element (according to {@link Object#equals}) after the first are ignored. This method iterates
   * over {@code elements} at most once.
   *
   * <p>
   * Note that if {@code s} is a {@code Set<String>}, then {@code ImmutableSet.copyOf(s)} returns an
   * {@code ImmutableSet<String>} containing each of the strings in {@code s}, while
   * {@code ImmutableSet.of(s)} returns a {@code ImmutableSet<Set<String>>} containing one element
   * (the given set itself).
   *
   * <p>
   ...
   *
   * <p>
   * <b>Note:</b> Despite what the method name suggests, {@code copyOf} will return constant-space
   * views, rather than linear-space copies, of some inputs known to be immutable. For some other
   * immutable inputs, such as key sets of an {@code ImmutableMap}, it still performs a copy in
   * order to avoid holding references to the values of the map. The heuristics used in this
   * decision are undocumented and subject to change except that:
   * <ul>
   * <li>A full copy will be done of any {@code ImmutableSortedSet}.</li>
   * <li>{@code ImmutableSet.copyOf()} is idempotent with respect to pointer equality.</li>
   * </ul>
   *
   * <p>
  ...
  public static <E> ImmutableSet<E> copyOf(Collection<? extends E> elements) {
    /*
     * TODO(user): consider checking for ImmutableAsList here TODO(user): consider checking for
     * Multiset here
     */
   ...
   * "logical contents" and they are reconstructed using public static factories. This is necessary
   * to ensure that the existence of a particular implementation type is an implementation detail.
   */
