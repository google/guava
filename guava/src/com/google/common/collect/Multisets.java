...
public final class Multisets {
   ...
   * {@code clear()} are called on the filtered multiset, only elements that satisfy the filter will
   * be removed from the underlying multiset.
   *
   * <p>
   ...
   * in the underlying multiset and determine which elements satisfy the filter. When a live view is
   * <i>not</i> needed, it may be faster to copy the returned multiset and use the copy.
   *
   * <p>
   ...
   * the members of the element set of {@code multiset2} that are not contained in {@code multiset1}
   ...
   *
   * <p>
   * This is <i>not</i> the same as {@code multisetToModify.} {@link Multiset#removeAll removeAll}
   * {@code (occurrencesToRemove)}, which removes all occurrences of elements that appear in
   * {@code occurrencesToRemove}. However, this operation <i>is</i> equivalent to, albeit sometimes
   ...
   *
   * @return {@code true} if {@code multisetToModify} was changed as a result of this operation
   * @since 18.0 (present in 10.0 with a requirement that the second parameter be a {@code Multiset}
   *        )
   */
  ...
  private static boolean removeOccurrencesImpl(Multiset<?> multisetToModify,
      Iterable<?> occurrencesToRemove) {
    checkNotNull(multisetToModify);
  ...
  static boolean equalsImpl(Multiset<?> multiset, @Nullable Object object) {
    ...
    if (object instanceof Multiset) {
      ...
      /*
       * We can't simply check whether the entry sets are equal, since that approach fails when a
       * TreeMultiset has a comparator that returns 0 when passed unequal elements.
       */
