...
abstract class AbstractMapBasedMultimap<K, V> extends AbstractMultimap<K, V> implements
   ...
   * delegate in the WrappedCollection is empty, the multimap may contain subsequently added values
   * for that key. To handle that situation, the WrappedCollection checks whether map contains an
   * entry for the provided key, and if so replaces the delegate.
   */
   ...
   *
   * <p>
   * Full collections, identified by a null ancestor field, contain all multimap values for a given
   * key. Its delegate is a value in {@link AbstractMapBasedMultimap#map} whenever the delegate is
   * non-empty. The {@code refreshIfEmpty}, {@code removeIfEmpty}, and {@code addToMap} methods
   ...
   * {@code refreshIfEmpty}, {@code removeIfEmpty}, and {@code addToMap} methods call the
  ...
  /**
   * Removes all values for the provided key. Unlike {@link #removeAll}, it returns the number of
   * removed mappings.
   */
   ...
   *
   * <p>
   * The iterator generated by the returned collection traverses the values for one key, followed by
   * the values of a second key, and so on.
   */
  @Override
   ...
   * LinkedHashMultimap that need to say something different are still able to {@inheritDoc} all the
   * way from Multimap?
   */
