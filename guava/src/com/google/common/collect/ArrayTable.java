...
public final class ArrayTable<R, C, V> extends AbstractTable<R, C, V> implements Serializable {
   ...
   * allowed row keys and/or column keys. Note that custom serialization logic is needed to support
   * different enum sizes during serialization and deserialization.
   */
   ...
   * {@code table.rowKeySet()} and the column keys in {@code table.columnKeySet()}. Calling
   * {@link #put} with other keys leads to an {@code IllegalArgumentException}.
   *
   * <p>
   * The ordering of {@code table.rowKeySet()} and {@code table.columnKeySet()} determines the row
   * and column iteration ordering of the returned table.
   *
   * @throws NullPointerException if {@code table} has a null key
   * @throws IllegalArgumentException if the provided table is empty
   */
  public static <R, C, V> ArrayTable<R, C, V> create(Table<R, C, V> table) {
    return (table instanceof ArrayTable<?, ?, ?>) ? new ArrayTable<R, C, V>(
        (ArrayTable<R, C, V>) table) : new ArrayTable<R, C, V>(table);
  }
   ...
   * @throws IndexOutOfBoundsException if either index is negative, {@code rowIndex} is greater then
   *         or equal to the number of allowed row keys, or {@code columnIndex} is greater then or
   *         equal to the number of allowed column keys
   */
   ...
   * {@code put(rowKeyList().get(rowIndex), columnKeyList().get(columnIndex), value)} has the same
   ...
   *
   * <p>
   * The returned collection's iterator traverses the values of the first row key, the values of the
   * second row key, and so on.
   *
