...
public final class ArrayTable<
  ...
  // The return type is "extremely" @Nullable, since this is a Table<R, C, @Nullable V>.
  @Override
  ...
  public @Nullable V put(R rowKey, C columnKey, @Nullable V value) {
    ...
    Integer rowIndex = rowKeyToIndex.get(rowKey);
    if (rowIndex == null) {
      throw new IllegalArgumentException("Row " + rowKey + " not in " + rowList);
    }
    Integer columnIndex = columnKeyToIndex.get(columnKey);
    if (columnIndex == null) {
      throw new IllegalArgumentException("Column " + columnKey + " not in " + columnList);
    }
