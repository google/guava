 ...
 *
 * <p>
 * The collections returned by {@code keySet}, {@code keys}, and {@code asMap} iterate through the
 * keys in the order they were first added to the multimap. Similarly, {@code get},
 * {@code removeAll}, and {@code replaceValues} return collections that iterate through the values
 * in the order they were added. The collections generated by {@code entries} and {@code values}
...
public final class LinkedHashMultimap<K, V> extends AbstractSetMultimap<K, V> {
   ...
   * hash table for a Set<V> associated with a key, the linked list of insertion-ordered entries in
   * that Set<V>, and the linked list of entries in the LinkedHashMultimap as a whole.
   */
  @VisibleForTesting
   ...
   *
   * <p>
   * Creates a decorated insertion-ordered set that also keeps track of the order in which key-value
   * pairs are added to the multimap.
   *
   ...
   *
   * <p>
   * If {@code values} is not empty and the multimap already contains a mapping for {@code key}, the
   * {@code keySet()} ordering is unchanged. However, the provided values always come last in the
   * {@link #entries()} and {@link #values()} iteration orderings.
   */
  @Override
  ...
  final class ValueSet extends Sets.ImprovedAbstractSet<V> implements ValueSetLink<K, V> {
    ...
    private void rehashIfNecessary() {
      if (Hashing.needsResizing(size, hashTable.length, VALUE_SET_LOAD_FACTOR)) {
        ...
        for (ValueSetLink<K, V> entry = firstEntry; entry != this; entry =
