...
@Beta final class SortedLists {
  ...
  public enum Relation {
    ...
    /**
     * The relation that specifies the greatest element strictly less than the value. In
     * the case of duplicates, the last such element is chosen.
     *
     * <p>In the {@link SortedLists#binarySearch binarySearch} methods, {@code -1} is
     * considered to be the index of negative infinity. That is, if there is no element in
     * the list lower than a given value, {@code -1} is returned as the index of the lower
    ...
    /**
     * The relation that specifies the greatest element less than or equal to the value.
     * In the case of duplicates, the first element equal to the value is chosen, or if no
     * such element exists, among the greatest elements less than or equal to the value,
     * the last is chosen.
     *
     * <p>In the {@link SortedLists#binarySearch binarySearch} methods, {@code -1} is
     * considered to be the index of negative infinity. That is, if there is no element in
     * the list less than or equal to a given value, {@code -1} is returned as the index
     * of the floor element.  For example: <pre> {@code
     *
     * Integer low = 1;
     * Integer mid = 2;
     * Integer high = 3;
    ...
    /**
     * The relation that specifies an element equal to the value.  In the case of
     * duplicates, no guarantee is made on which element is chosen.
     *
     * <p>In the {@link SortedLists#binarySearch binarySearch} methods, if there is no
     * element that compares as equal to a given value, {@code -1} is returned to
     * indicate failure. For example: <pre> {@code
     *
     * Integer low = 1;
     * Integer mid = 2;
     * Integer high = 3;
    ...
    /**
     * The relation that specifies the least element greater than or equal to the value.
     * In the case of duplicates, the last element equal to the value is chosen, or if no
     * such element exists, among the least elements greater than or equal to the value,
     * the first is chosen.
     *
     * <p>In the {@link SortedLists#binarySearch binarySearch} methods, {@code
     * list.size()} is considered to be the index of positive infinity. That is, if there
     * is no element in the list greater than or equal to a given value, {@code -1} is
     * returned as the index of the ceiling element. For example: <pre> {@code
     *
     * Integer low = 1;
     * Integer mid = 2;
     * Integer high = 3;
    ...
    /**
     * The relation that specifies the least element strictly greater than the value. In
     * the case of duplicates, the first such element is chosen.
     *
     * <p>In the {@link SortedLists#binarySearch binarySearch} methods, {@code
     * list.size()} is considered to be the index of positive infinity. That is, if there
     * is no element in the list greater than a given value, {@code list.size()} is
     * returned as the index of the higher element. For example: <pre> {@code
     *
     * Integer low = 1;
     * Integer mid = 2;
     * Integer high = 3;
    ...
    /**
     * The reverse order counterpart of the relation. Useful for descending views.
     */
    public abstract Relation reverse();

    /**
     * Given that {@code list.get(lower - 1) < list.get(index) = e < list.get(upper + 1)}
     * according to this comparator, find the index of the element with this relation.
     *
     * <p>{@code 0 <= lower <= index <= upper < list.size()}.
     */
    ...
    /**
     * Given that {@code list.get(higherIndex - 1) < e < list.get(higherIndex)} according
     * to this comparator, find the index of the element with this relation.
     *
     * <p>{@code 0 <= higherIndex <= list.size()}.
     */
   ...
   * search algorithm.
   *
   * <p>Equivalent to {@linkplain #binarySearch(List, Object, Comparator, Relation)
   * binarySearch}{@code (list, key, Ordering.natural(), relation)}.
   */
   ...
   * Collections.sort(List, Comparator)} method), prior to making this call. If it is not
   * sorted, the results are undefined.
   *
   * <p>Returns the index of the element in the list which has the specified {@code
   * Relation} to the specified object. So as to provide meaningful results in all cases,
   * {@code -1} is considered to be the index of negative infinity, and {@code
   * list.size()} is considered to be the index of positive infinity. The exception is
   * {@link Relation#EQUAL EQUAL}. If {@code EQUAL} is specified and no equal element is
   * found, {@code -1} is returned, but it should not be interpreted as "negative
   * infinity."
   *
   * <p>If there are duplicate elements, see the documentation on the relation for more
   * details.
   *
   * <p>This method runs in log(n) time on random-access lists, which offer1G
   * near-constant-time access to each list element.
   *
   * @param list the list to be searched.
   * @param e the value to be searched for.
   * @param comparator the comparator by which the list is ordered.
   * @return the index of element with the specified relation to the search key, if it is
   *     contained in the list. Otherwise, if negative infinity has the specified relation
   *     (or if {@code relation} is {@code EQUAL} and the search key was not in the list),
   *     returns {@code -1}, or if positive infinity has the specified relation, returns
   *     {@code list.size()}.
   * @throws NullPointerException if {@code e} is null and the specified comparator does
