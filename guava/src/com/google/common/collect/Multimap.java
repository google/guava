 ...
 * <li>Many complex operations become easier; for example,
 * {@code Collections.min(multimap.values())} finds the smallest value across all keys.
 * </ul>
 *
 ...
 * Known Implementing Classes". You can also create a <i>custom</i> multimap, backed by any
 * {@code Map} and {@link Collection} types, using the {@link Multimaps#newMultimap
 * Multimaps.newMultimap} family of methods. Finally, another popular way to obtain a multimap is
 * using {@link Multimaps#index Multimaps.index}. See the {@link Multimaps} class for these and
 * other static utilities related to multimaps.
 *
 * <h3>Other Notes</h3>
 * 
 * <p>
...
public interface Multimap<K, V> {
   ...
   *
   * <p>
   * Some multimap implementations allow duplicate key-value pairs, in which case {@code put} always
   * adds a new key-value pair and increases the multimap size by 1. Other implementations prohibit
   * duplicates, and storing a key-value pair that's already in the multimap has no effect.
   *
   ...
   *         associated with the key. The collection <i>may</i> be modifiable, but updating it will
   *         have no effect on the multimap.
   */
   ...
   *
   * <p>
   * Changes to the returned multiset will update the underlying multimap, and vice versa. However,
   * <i>adding</i> to the returned collection is not possible.
   */
  Multiset<K> keys();
   ...
   *
   * <p>
   * In general, two multimaps with identical key-value mappings may or may not be equal, depending
   * on the implementation. For example, two {@link SetMultimap} instances with the same key-value
   * mappings are equal, but equality of two {@link ListMultimap} instances depends on the ordering
   ...
   * codes, depending on the implementation. For example, two {@link SetMultimap} instances with the
   * same key-value mappings will have the same {@code hashCode}, but the {@code hashCode} of
