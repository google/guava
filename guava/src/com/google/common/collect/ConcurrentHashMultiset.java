...
public final class ConcurrentHashMultiset<E> extends AbstractMultiset<E> implements Serializable {
   ...
   *
   * <p>
   * If this {@link MapMaker} is configured to use entry eviction of any kind, this eviction applies
   * to all occurrences of a given element as a single unit. However, most updates to the multiset
   * do not count as map updates at all, since we're usually just mutating the value stored in the
   * map, so {@link MapMaker#expireAfterAccess} makes sense (evict the entry that was queried or
   * updated longest ago), but {@link MapMaker#expireAfterWrite} doesn't, because the eviction time
   * is measured from when we saw the first occurrence of the object.
   *
   * <p>
   * The returned multiset is serializable but any serialization caveats given in {@code MapMaker}
   * apply.
   *
   * <p>
   ...
   * This satisfies both NullPointerTester and CollectionRemoveTester.testRemove_nullAllowed, but
   * it's not clear that it's a good policy, especially because, in order for the test to pass, the
   * parameter must be misleadingly annotated as @Nullable. I suspect that we'll want to remove
   * @Nullable, add an eager checkNotNull, and loosen up testRemove_nullAllowed.
   */
  @Override
   ...
   *
   * <p>
   * This method, in contrast to {@link #remove(Object, int)}, has no effect when the element count
   * is smaller than {@code occurrences}.
   *
   ...
   *         been modified, but not always: in the case that {@code expectedOldCount == newCount},
   *         the method will return {@code true} if the condition was met.
