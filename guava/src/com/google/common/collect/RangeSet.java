...
public abstract class RangeSet<C extends Comparable> {
  ...
  /**
   * Returns the range with the lowest lower bound in this range set.
   * 
   * @throws NoSuchElementException if this range set is empty
   */
  public Range<C> firstRange() {
    return asRanges().iterator().next();
  }

  /**
   * Returns the range with the greatest upper bound in this range set.
   * 
   * @throws NoSuchElementException if this range set is empty
   */
  public Range<C> lastRange() {
    return Iterators.getLast(asRanges().iterator());
  }

  /**
   * Returns the minimal range that {@linkplain Range#encloses encloses} every one of the disjoint
