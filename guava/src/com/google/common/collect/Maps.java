 ...
 * "http://code.google.com/p/guava-libraries/wiki/CollectionUtilitiesExplained#Maps"> {@code Maps}
 * </a>.
 *
...
public final class Maps {
   ...
   *
   * @param expectedSize the number of elements you expect to add to the returned map
   * @return a new, empty {@code HashMap} with enough capacity to hold {@code expectedSize} elements
   *         without resizing
   ...
   *
   * <p>
   * Since this method uses {@code HashMap} instances internally, the keys of the supplied maps must
   * be well-behaved with respect to {@link Object#equals} and {@link Object#hashCode}.
   *
   * <p>
  ...
  private static <K, V> void doDifference(Map<? extends K, ? extends V> left,
      Map<? extends K, ? extends V> right, Equivalence<? super V> valueEquivalence,
   ...
   *
   * <p>
   * Since this method uses {@code TreeMap} instances internally, the keys of the right map must all
   * compare as distinct according to the comparator of the left map.
   *
   * <p>
  ...
  static class SortedMapDifferenceImpl<K, V> extends MapDifferenceImpl<K, V> implements
    ...
    SortedMapDifferenceImpl(SortedMap<K, V> onlyOnLeft, SortedMap<K, V> onlyOnRight,
   ...
   * computed on demand using {@code function}. To get an immutable <i>copy</i> instead, use
   * {@link #toMap(Iterable, Function)}.
   *
   * <p>
   * Specifically, for each {@code k} in the backing set, the returned map has an entry mapping
   * {@code k} to {@code function.apply(k)}. The {@code keySet}, {@code values}, and
   * {@code entrySet} views of the returned map iterate in the same order as the backing set.
   *
   * <p>
   ...
   *
   * <p>
   * <b>Warning:</b> If the function rejects {@code null}, caution is required to make sure the set
   * does not contain {@code null}, because the view cannot stop {@code null} from being added to
   * the set.
   *
   * <p>
  ...
  static <K, V> Iterator<Entry<K, V>> asMapEntryIterator(Set<K> set,
      final Function<? super K, V> function) {
   ...
   * "read through" to the specified bimap, and attempts to modify the returned map, whether direct
   ...
   *
   * <p>
   * It's acceptable for the underlying map to contain null keys, and even null values provided that
   * the function is capable of accepting null input. The transformed map might contain null values,
   * if the function sometimes gives a null result.
   *
   * <p>
   * The returned map is not thread-safe or serializable, even if the underlying map is.
   *
   * <p>
   * The function is applied lazily, invoked when needed. This is necessary for the returned map to
   * be a view, but it means that the function will be applied many times for bulk operations like
   * {@link Map#containsValue} and {@code Map.toString()}. For this to perform well,
   * {@code function} should be fast. To avoid lazy evaluation when the returned map doesn't need to
   ...
   * transformer is capable of accepting null inputs. The transformed map might contain null values
   * if the transformer sometimes gives a null result.
   *
   * <p>
   * The returned map is not thread-safe or serializable, even if the underlying map is.
   *
   * <p>
   * The transformer is applied lazily, invoked when needed. This is necessary for the returned map
   * to be a view, but it means that the transformer will be applied many times for bulk operations
   * like {@link Map#containsValue} and {@link Object#toString}. For this to perform well,
   * {@code transformer} should be fast. To avoid lazy evaluation when the returned map doesn't need
   * to be a view, copy the returned map into a new map of your choosing.
   *
   * <p>
   * <b>Warning:</b> This method assumes that for any instance {@code k} of {@code EntryTransformer}
   * key type {@code K}, {@code k.equals(k2)} implies that {@code k2} is also of type {@code K}.
   * Using an {@code EntryTransformer} key type for which this may not hold, such as
   ...
   *
   * <p>
   * The resulting map's {@code keySet()}, {@code entrySet()}, and {@code values()} views have
   ...
   *
   * <p>
   * When methods such as {@code removeAll()} and {@code clear()} are called on the filtered map or
   * its views, only mappings whose keys satisfy the filter will be removed from the underlying map.
   *
   * <p>
   ...
   * or its views, only mappings that satisfy the filter will be removed from the underlying bimap.
   *
   * <p>
   ...
   * underlying bimap and determine which satisfy the filter. When a live view is <i>not</i> needed,
   * it may be faster to copy the filtered bimap and use the copy.
   *
   * <p>
   * <b>Warning:</b> {@code entryPredicate} must be <i>consistent with equals </i>, as documented at
   * {@link Predicate#apply}.
   *
   * @since 14.0
   */
   ...
   * <p>
   * When methods such as {@code removeAll()} and {@code clear()} are called on the filtered map or
   * its views, only mappings whose values satisfy the filter will be removed from the underlying
   * map.
   *
   * <p>
   * The returned map isn't threadsafe or serializable, even if {@code unfiltered} is.
   ...
   * The resulting bimap's {@code keySet()}, {@code entrySet()}, and {@code values()} views have
   * iterators that don't support {@code remove()}, but all other methods are supported by the bimap
   * and its views. When given a value that doesn't satisfy the predicate, the bimap's {@code put()}, {@code forcePut()} and {@code putAll()} methods throw an {@link IllegalArgumentException}.
   * Similarly, the map's entries have a {@link Entry#setValue} method that throws an
   * {@link IllegalArgumentException} when the provided value doesn't satisfy the predicate.
   *
   * <p>
   * When methods such as {@code removeAll()} and {@code clear()} are called on the filtered bimap
   * or its views, only mappings that satisfy the filter will be removed from the underlying bimap.
   ...
   *
   * <p>
   * Many of the filtered bimap's methods, such as {@code size()}, iterate across every value in the
   * underlying bimap and determine which satisfy the filter. When a live view is <i>not</i> needed,
   * it may be faster to copy the filtered bimap and use the copy.
   *
   * <p>
   ...
   * {@code put()} and {@code putAll()} methods throw an {@link IllegalArgumentException}.
   * Similarly, the map's entries have a {@link Entry#setValue} method that throws an
   * {@link IllegalArgumentException} when the existing key and the provided value don't satisfy the
   * predicate.
   *
   * <p>
   * When methods such as {@code removeAll()} and {@code clear()} are called on the filtered map or
   * its views, only mappings that satisfy the filter will be removed from the underlying map.
   *
   * <p>
   * The returned map isn't threadsafe or serializable, even if {@code unfiltered} is.
   *
  ...
  private static <K, V> Map<K, V> filterFiltered(AbstractFilteredMap<K, V> map,
      Predicate<? super Entry<K, V>> entryPredicate) {
    return new FilteredEntryMap<K, V>(map.unfiltered, Predicates.<Entry<K, V>>and(map.predicate,
        entryPredicate));
  }
  ...
  private static class FilteredEntryNavigableMap<K, V> extends AbstractNavigableMap<K, V> {
    ...
    @Override
    public NavigableMap<K, V> subMap(K fromKey, boolean fromInclusive, K toKey, boolean toInclusive) {
      return filterEntries(unfiltered.subMap(fromKey, fromInclusive, toKey, toInclusive),
          entryPredicate);
    }
  ...
  static class UnmodifiableNavigableMap<K, V> extends ForwardingSortedMap<K, V> implements
    ...
    public NavigableMap<K, V> descendingMap() {
      ...
      return (result == null) ? descendingMap =
          new UnmodifiableNavigableMap<K, V>(delegate.descendingMap(), this) : result;
    }
  ...
  /**
   * Delegates to {@link Map#get}. Returns {@code null} on {@code ClassCastException} and
  ...
  /**
   * Delegates to {@link Map#remove}. Returns {@code null} on {@code ClassCastException} and
   * {@code NullPointerException}.
   */
