 ...
 *
 * <p>
 * The methods {@link #get}, {@link #keySet()}, {@link #keys()}, {@link #values}, {@link #entries()}
 * , and {@link #asMap} return collections that are views of the multimap. If the multimap is
 * modified while an iteration over any of those collections is in progress, except through the
...
public class LinkedListMultimap<K, V> extends AbstractMultimap<K, V> implements ListMultimap<K, V>,
    Serializable {
  /*
   * Order is maintained using a linked list containing all key-value pairs. In addition, a series
   * of disjoint linked lists of "siblings", each containing the values for a specific key, is used
   * to implement {@link ValueForKeyIterator} in constant time.
   */
   ...
   *
   * <p>
   * If the multimap is modified while an iteration over the list is in progress (except through the
   * iterator's own {@code add}, {@code set} or {@code remove} operations) the results of the
   ...
   *
   * <p>
   * The iterator generated by the returned collection traverses the entries in the order they were
   * added to the multimap. Because the entries may have duplicates and follow the insertion
   * ordering, this method returns a {@link List}, instead of the {@link Collection} specified in
   * the {@link ListMultimap} interface.
   *
   * <p>
   * An entry's {@link Entry#getKey} method always returns the same key, regardless of what happens
   * subsequently. As long as the corresponding key-value mapping is not removed from the multimap,
   * {@link Entry#getValue} returns the value from the multimap, which may change over time, and
   * {@link Entry#setValue} modifies that value. Removing the mapping from the multimap does not
   * alter the value returned by {@code getValue()}, though a subsequent {@code setValue()} call
   * won't update the multimap but will lead to a revised value being returned by {@code getValue()}.
   */
  @Override
