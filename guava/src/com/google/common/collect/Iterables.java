...
public final class Iterables {
   ...
   * (that is, identical to {@link java.util.Arrays Arrays}
   * {@code .toString(Iterables.toArray(iterable))}). Note that for <i>most</i> implementations of
   * {@link Collection}, {@code collection.toString()} also gives the same result, but that behavior
   ...
   * an outer iterable containing two inner lists of three elements each, all in the original order.
   *
   * <p>
   * Iterators returned by the returned iterable do not support the {@link Iterator#remove()}
   * method.
  ...
  /**
   * Returns {@code true} if every element in {@code iterable} satisfies the predicate. If
   * {@code iterable} is empty, {@code true} is returned.
   */
   ...
   * {@code defaultValue} if none found. Note that this can usually be handled more naturally using
   * {@code tryFind(iterable, predicate).or(defaultValue)}.
   *
   * @since 7.0
   */
  @Nullable
   ...
   * matched in {@code iterable}, a NullPointerException will be thrown.
   *
   * @since 11.0
   */
   ...
   *
   * <p>
   * The returned iterable's iterator supports {@code remove()} if the provided iterator does. After
  ...
  public static <T> T get(Iterable<T> iterable, int position) {
    checkNotNull(iterable);
    return (iterable instanceof List) ? ((List<T>) iterable).get(position) : Iterators.get(
        iterable.iterator(), position);
  }
   ...
   * to be thrown), it is recommended that {@code iterable.iterator().next()} is used instead.
   *
  ...
  public static <T> Iterable<T> skip(final Iterable<T> iterable, final int numberToSkip) {
    ...
    return new FluentIterable<T>() {
      ...
      public Iterator<T> iterator() {
        ...
        /*
         * We can't just return the iterator because an immediate call to its remove() method would
         * remove one of the skipped elements instead of throwing an IllegalStateException.
         */
   ...
   * {@link Iterator#hasNext()} on a generated iterator from the returned iterable may cause an item
   * to be immediately dequeued for return on a subsequent call to {@link Iterator#next()}.
   *
