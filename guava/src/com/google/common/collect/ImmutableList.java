...
/**
 * A high-performance, immutable, random-access {@code List} implementation. Does not permit null
 * elements.
 *
 * <p>
 * Unlike {@link Collections#unmodifiableList}, which is a <i>view</i> of a separate collection that
 * can still change, an instance of {@code ImmutableList} contains its own private data and will
 * <i>never</i> change. {@code ImmutableList} is convenient for {@code public static final} lists
 * ("constant lists") and also lets you easily make a "defensive copy" of a list provided to your
 * class by a caller.
 *
 * <p>
...
public abstract class ImmutableList<E> extends ImmutableCollection<E> implements List<E>,
  ...
  private static final ImmutableList<Object> EMPTY = new RegularImmutableList<Object>(
      ObjectArrays.EMPTY_ARRAY);

  /**
   * Returns the empty immutable list. This set behaves and performs comparably to
   * {@link Collections#emptyList}, and is preferable mainly for consistency and maintainability of
   * your code.
   */
   ...
   *
   * <p>
   * Note that if {@code list} is a {@code List<String>}, then {@code ImmutableList.copyOf(list)}
   ...
   * {@code ImmutableList.of(1, 2, 3).reverse()} is equivalent to {@code ImmutableList.of(3, 2, 1)}.
   *
  ...
  /*
   * Serializes ImmutableLists as their logical contents. This ensures that implementation types do
   * not leak into the serialized representation.
   */
