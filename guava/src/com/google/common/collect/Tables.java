...
public final class Tables {
   ...
   *
   * <p>
   * The returned table supports update operations as long as the input table supports the analogous
   * operation with swapped rows and columns. For example, in a {@link HashBasedTable} instance,
   * {@code rowKeySet().iterator()} supports {@code remove()} but {@code columnKeySet().iterator()}
   * doesn't. With a transposed {@link HashBasedTable}, it's the other way around.
   */
   ...
   *
   * <p>
   * Note: the table assumes complete ownership over of {@code backingMap} and the maps returned by
   ...
   * {@code function} should be fast. To avoid lazy evaluation when the returned table doesn't need
   * to be a view, copy the returned table into a new table of your choosing.
   *
   * @since 10.0
   */
  @Beta
