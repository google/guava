...
abstract class Dispatcher {
   ...
   *
   * <p>
   * When all subscribers are dispatched to using a <i>direct</i> executor (which dispatches on the
   * same thread that posts the event), this yields a breadth-first dispatch order on each thread.
   * That is, all subscribers to a single event A will be called before any subscribers to any
   * events B and C that are posted to the event bus by the subscribers to A.
   */
  ...
  private static final class LegacyAsyncDispatcher extends Dispatcher {
     ...
     * Global event queue.
     */
    private final ConcurrentLinkedQueue<EventWithSubscriber> queue = Queues
