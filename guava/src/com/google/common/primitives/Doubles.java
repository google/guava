 ...
 * "http://code.google.com/p/guava-libraries/wiki/PrimitivesExplained"> primitive utilities</a>.
 *
...
public final class Doubles {
   ...
   * that of <code>((Double) a).{@linkplain
   * Double#compareTo compareTo}(b)</code>. As with that method, {@code NaN} is treated as greater
   * than all other values, and {@code 0.0 > -0.0}.
   *
   * <p>
  ...
  private static final class DoubleConverter extends Converter<String, Double> implements
      Serializable {
   ...
   *
   * <p>
   * Note that {@link Double#toString(double)} formats {@code double} differently in GWT sometimes.
   ...
   *
   * <p>
   * Elements are copied from the argument collection as if by {@code collection.toArray()}. Calling
   * this method is as thread-safe as calling that method.
   *
   ...
   *
   * <p>
   * The returned list may have unexpected behavior if it contains {@code NaN}, or if {@code NaN} is
   ...
   *
   * <p>
   * Unlike {@link Double#parseDouble(String)}, this method returns {@code null} instead of throwing
   ...
   *
   * <p>
   * This implementation is likely to be faster than {@code Double.parseDouble} if many failures are
   * expected.
   *
