...
public final class BloomFilter<T> implements Predicate<T>, Serializable {
  ...
  public boolean isCompatible(BloomFilter<T> that) {
    ...
    return (this != that) && (this.numHashFunctions == that.numHashFunctions)
        && (this.bitSize() == that.bitSize()) && (this.strategy.equals(that.strategy))
  ...
  public void putAll(BloomFilter<T> that) {
    ...
    checkArgument(this.strategy.equals(that.strategy),
        "BloomFilters must have equal strategies (%s != %s)", this.strategy, that.strategy);
    checkArgument(this.funnel.equals(that.funnel),
        "BloomFilters must have equal funnels (%s != %s)", this.funnel, that.funnel);
  ...
  static <T> BloomFilter<T> create(Funnel<? super T> funnel, long expectedInsertions, double fpp,
    ...
    /*
     * TODO(user): Put a warning in the javadoc about tiny fpp values, since the resulting size is
     * proportional to -log(p), but there is not much of a point after all, e.g. optimalM(1000,
     * 0.0000000000000001) = 76680 which is less than 10kb. Who cares!
     */
   ...
   * Cheat sheet:
   * 
   * m: total bits n: expected insertions b: m/n, bits per insertion p: expected false positive
   * probability
   * 
   * 1) Optimal k = b * ln2 2) p = (1 - e ^ (-kn/m))^k 3) For optimal k: p = 2 ^ (-k) ~= 0.6185^b 4)
   * For optimal k: m = -nlnp / ((ln2) ^ 2)
   */
  ...
  public void writeTo(OutputStream out) throws IOException {
    /*
     * Serial form: 1 signed byte for the strategy 1 unsigned byte for the number of hash functions
     * 1 big endian int, the number of longs in our bitset N big endian longs of our bitset
     */
  ...
  public static <T> BloomFilter<T> readFrom(InputStream in, Funnel<T> funnel) throws IOException {
    ...
    try {
      ...
      IOException ioException =
          new IOException("Unable to deserialize BloomFilter from InputStream."
              + " strategyOrdinal: " + strategyOrdinal + " numHashFunctions: " + numHashFunctions
