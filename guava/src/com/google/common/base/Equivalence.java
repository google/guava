...
public abstract class Equivalence<T> {
   ...
   * {@code equivalent(x, x)} returns {@code true}.
   ...
   * {@code equivalent(x, y)} returns {@code true} and {@code equivalent(y, z)} returns {@code true}
   * , then {@code equivalent(x, z)} returns {@code true}.
   ...
   * if {@code equivalent(x, y)}, then {@code hash(x) == hash(y)}. It is <i>not</i> necessary that
   * the hash be distributable across <i>inequivalence</i>. If {@code equivalence(x, y)} is false,
   * {@code hash(x) == hash(y)} may still be true.
   ...
   * non-null objects {@code x} and {@code y},
   * {@code equivalence.onResultOf(function).equivalent(a, b)} is true if and only if
