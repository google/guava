 ...
 * on <a href= "http://code.google.com/p/guava-libraries/wiki/ListenableFutureExplained">
...
public final class Futures {
   ...
   *
   * <p>
   * The returned {@code Future} can't be cancelled, and its {@code isDone()} method always returns
   * {@code true}. Calling {@code get()} or {@code checkedGet()} will immediately return the
   * provided value.
   */
  @CheckReturnValue
  @GwtIncompatible("TODO")
   ...
   * <p>
   * The returned {@code Future} can't be cancelled, and its {@code isDone()} method always returns
   * {@code true}. Calling {@code get()} will immediately throw the provided {@code Throwable}
   ...
   * <p>
   * The returned {@code Future} can't be cancelled, and its {@code isDone()} method always returns
   * {@code true}. Calling {@code get()} will immediately throw the provided {@code Exception}
   * wrapped in an {@code ExecutionException}, and calling {@code checkedGet()} will throw the
   ...
   * {@link FutureFallback#create} is not invoked until the primary input has failed, so if the
   ...
   *
   * <p>
   * Note: If the derived {@code Future} is slow or heavyweight to create (whether the
   * {@code Future} itself is slow or heavyweight to complete is irrelevant), consider
   * {@linkplain #withFallback(ListenableFuture, FutureFallback, Executor) supplying an executor}.
   * If you do not supply an executor, {@code withFallback} will use a
   * {@linkplain MoreExecutors#directExecutor direct executor}, which carries some caveats for
   * heavier operations. For example, the call to {@code fallback.create} may run on an
   * unpredictable or undesirable thread:
   *
   * <ul>
   * <li>If the input {@code Future} is done at the time {@code withFallback} is called,
   * {@code withFallback} will call {@code fallback.create} inline.
   * <li>If the input {@code Future} is not yet done, {@code withFallback} will schedule
   * {@code fallback.create} to be run by the thread that completes the input {@code Future}, which
   * may be an internal system thread such as an RPC network thread.
   * </ul>
   *
   * <p>
   * Also note that, regardless of which thread executes the {@code fallback.create}, all other
   ...
   *
   * <p>
   * When the execution of {@code fallback.create} is fast and lightweight (though the
   * {@code Future} it returns need not meet these criteria), consider
   * {@linkplain #withFallback(ListenableFuture, FutureFallback) omitting the executor} or
   * explicitly specifying {@code directExecutor}. However, be aware of the caveats documented in
   * the link above.
   *
   * @param input the primary input {@code Future}
   * @param fallback the {@link FutureFallback} implementation to be called if {@code input} fails
  ...
  public static <V> ListenableFuture<V> withTimeout(ListenableFuture<V> delegate, long time,
   ...
   * Note: If the derived {@code Future} is slow or heavyweight to create (whether the
   * {@code Future} itself is slow or heavyweight to complete is irrelevant), consider
   * {@linkplain #transform(ListenableFuture, AsyncFunction, Executor) supplying an executor}. If
   * you do not supply an executor, {@code transform} will use a
   * {@linkplain MoreExecutors#directExecutor direct executor}, which carries some caveats for
   * heavier operations. For example, the call to {@code function.apply} may run on an unpredictable
   * or undesirable thread:
   *
   * <ul>
   * <li>If the input {@code Future} is done at the time {@code transform} is called,
   * {@code transform} will call {@code function.apply} inline.
   * <li>If the input {@code Future} is not yet done, {@code transform} will schedule
   * {@code function.apply} to be run by the thread that completes the input {@code Future}, which
   * may be an internal system thread such as an RPC network thread.
   * </ul>
   *
   * <p>
   * Also note that, regardless of which thread executes the {@code function.apply}, all other
   * registered but unexecuted listeners are prevented from running during its execution, even if
   * those listeners are to run in other executors.
   *
   * <p>
   * The returned {@code Future} attempts to keep its cancellation state in sync with that of the
   * input future and that of the future returned by the function. That is, if the returned
   ...
   *
   * <p>
   * When the execution of {@code function.apply} is fast and lightweight (though the {@code Future}
   * it returns need not meet these criteria), consider
   * {@linkplain #transform(ListenableFuture, AsyncFunction) omitting the executor} or explicitly
   ...
   */
  private static Runnable rejectionPropagatingRunnable(final AbstractFuture<?> outputFuture,
      final Runnable delegateTask, final Executor delegateExecutor) {
    return new Runnable() {
      @Override
      public void run() {
  ...
  /**
   * Returns a new {@code ListenableFuture} whose result is the product of applying the given
   * {@code Function} to the result of the given {@code Future}. Example:
   *
   * <pre>
   * {@code
   ...
   *
   * <p>
   * Note: If the transformation is slow or heavyweight, consider
   * {@linkplain #transform(ListenableFuture, Function, Executor) supplying an executor}. If you do
   * not supply an executor, {@code transform} will use an inline executor, which carries some
   * caveats for heavier operations. For example, the call to {@code function.apply} may run on an
   * unpredictable or undesirable thread:
   *
   * <ul>
   * <li>If the input {@code Future} is done at the time {@code transform} is called,
   ...
   *
   * <p>
   * When the transformation is fast and lightweight, consider
   * {@linkplain #transform(ListenableFuture, Function) omitting the executor} or explicitly
   * specifying {@code directExecutor}. However, be aware of the caveats documented in the link
   * above.
   *
   * @param input The future to transform
  ...
  /**
   * Like {@link #transform(ListenableFuture, Function)} except that the transformation
   ...
   *
   * <p>
   * Note that calls to {@linkplain Future#get(long, TimeUnit) timed get} only apply the timeout to
   * the execution of the underlying {@code Future}, <em>not</em> to the execution of the
   * transformation function.
   *
   * <p>
   * The primary audience of this method is callers of {@code transform} who don't have a
   * {@code ListenableFuture} available and do not mind repeated, lazy function evaluation.
   *
   * @param input The future to transform
   * @param function A Function to transform the results of the provided future to the results of
   *        the returned future.
  ...
  /**
   * An implementation of {@code ListenableFuture} that also implements {@code Runnable} so that it
   ...
   *
   * <p>
   * For historical reasons, this class has a special case in its exception handling: If the given
   * {@code AsyncFunction} throws an {@code UndeclaredThrowableException},
   * {@code ChainingListenableFuture} unwraps it and uses its <i>cause</i> as the output future's
   * exception, rather than using the {@code UndeclaredThrowableException} itself as it would for
   * other exception types. The reason for this is that {@code Futures.transform} used to require a
   * {@code Function}, whose {@code apply} method is not allowed to throw checked exceptions.
   * Nowadays, {@code Futures.transform} has an overload that accepts an {@code AsyncFunction},
   * whose {@code apply} method <i>is</i> allowed to throw checked exception. Users who wish to
   * throw checked exceptions should use that overload instead, and <a
   * href="http://code.google.com/p/guava-libraries/issues/detail?id=1548">we should remove the
   * {@code UndeclaredThrowableException} special case</a>.
   */
  private static final class ChainingListenableFuture<I, O> extends AbstractFuture.TrustedFuture<O>
      implements Runnable {
    ...
    public boolean cancel(boolean mayInterruptIfRunning) {
      /*
       * Our additional cancellation work needs to occur even if !mayInterruptIfRunning, so we can't
       * move it into interruptTask().
       */
      if (super.cancel(mayInterruptIfRunning)) {
   ...
   * {@link #transform(ListenableFuture, AsyncFunction)}, in that the returned {@code Future}
   * attempts to keep its cancellation state in sync with both the input {@code Future} and the
   * nested {@code Future}. The transformation is very lightweight and therefore takes place in the
   * same thread (either the thread that called {@code dereference}, or the thread in which the
   ...
   */
  @GwtIncompatible("TODO")
  private static final class NonCancellationPropagatingFuture<V> extends
   ...
   * {@code null} (which is indistinguishable from the future having a successful value of
   * {@code null}).
   *
   * <p>
   ...
   *
   * <p>
   * Cancelling a delegate future has no effect on any input future, since the delegate future does
   * not correspond to a specific input future until the appropriate number of input futures have
   * completed. At that point, it is too late to cancel the input future. The input future's result,
   * which cannot be stored into the cancelled delegate future, is ignored.
   *
   * @since 17.0
   */
  @Beta
   ...
   *
   * <p>
   * There is no guaranteed ordering of execution of callbacks, but any callback added through this
   * method is guaranteed to be called once the computation is complete.
   *
   * Example:
   * 
   * <pre>
   ...
   *
   * <p>
   * Note: If the callback is slow or heavyweight, consider
   * {@linkplain #addCallback(ListenableFuture, FutureCallback, Executor) supplying an executor}. If
   * you do not supply an executor, {@code addCallback} will use a
   * {@linkplain MoreExecutors#directExecutor direct executor}, which carries some caveats for
   * heavier operations. For example, the callback may run on an unpredictable or undesirable
   * thread:
   *
   * <ul>
   * <li>If the input {@code Future} is done at the time {@code addCallback} is called,
   * {@code addCallback} will execute the callback inline.
   * <li>If the input {@code Future} is not yet done, {@code addCallback} will schedule the callback
   * to be run by the thread that completes the input {@code Future}, which may be an internal
   * system thread such as an RPC network thread.
   * </ul>
   *
   * <p>
   * Also note that, regardless of which thread executes the callback, all other registered but
   * unexecuted listeners are prevented from running during its execution, even if those listeners
   ...
   * {@linkplain #addCallback(ListenableFuture, FutureCallback) omitting the executor} or explicitly
   * specifying {@code directExecutor}. However, be aware of the caveats documented in the link
   * above.
   *
   * <p>
   ...
   * a checked exception, an {@link UncheckedExecutionException} if the cause is a
   * {@code RuntimeException}, or an {@link ExecutionError} if the cause is an {@code Error}.
   ...
   * {@link RuntimeException} (though {@code get} implementations are discouraged from throwing such
   * exceptions).
   * </ul>
   *
   * <p>
   * The overall principle is to continue to treat every checked exception as a checked exception,
   * every unchecked exception as an unchecked exception, and every error as an error. In addition,
   * the cause of any {@code ExecutionException} is wrapped in order to ensure that the new stack
   * trace matches that of the current thread.
   *
   * <p>
   ...
   * {@link Throwable#initCause(Throwable)} on it. If no such constructor exists, an
   ...
   * {@code InterruptedException}, to pass through {@code CancellationException}, and to wrap any
   * exception from the underlying computation in an {@code UncheckedExecutionException} or
   * {@code ExecutionError}.
   *
   * <p>
   ...
   * the (static-method) Futures.get documentation
   */
  ...
  private static <V> ListenableFuture<List<V>> listFuture(
    ...
    return new CollectionFuture<V, List<V>>(futures, allMustSucceed, listenerExecutor,
