...
public final class MoreExecutors {
   ...
   *
   * <p>
   * This method waits 120 seconds before continuing with JVM termination, even if the executor has
  ...
  /**
   * Converts the given ThreadPoolExecutor into a ScheduledExecutorService that exits when the
   * application is complete. It does so by using daemon threads and adding a shutdown hook to wait
   ...
   *
   * <p>
   * This should be preferred to {@link #newDirectExecutorService()} because the implementing the
   ...
   * as {@code execute} and {@code invokeAny}, are implemented in terms of calls to
   * {@code delegate.execute}. All other methods are forwarded unchanged to the delegate. This
   * implies that the returned {@code ListeningExecutorService} never calls the delegate's
   * {@code submit}, {@code invokeAll}, and {@code invokeAny} methods, so any special handling of
   * tasks must be implemented in the delegate's {@code execute} method or by wrapping the returned
   * {@code ListeningExecutorService}.
   *
   * <p>
   ...
   * well as {@code execute} and {@code invokeAny}, are implemented in terms of calls to
   * {@code delegate.execute}. All other methods are forwarded unchanged to the delegate. This
   * implies that the returned {@code ListeningScheduledExecutorService} never calls the delegate's
   * {@code submit}, {@code invokeAll}, and {@code invokeAny} methods, so any special handling of
   * tasks must be implemented in the delegate's {@code execute} method or by wrapping the returned
   * {@code ListeningScheduledExecutorService}.
   *
   * <p>
   * If the delegate executor was already an instance of {@code ListeningScheduledExecutorService},
   * it is returned untouched, and the rest of this documentation does not apply.
   *
   * @since 10.0
   */
  @GwtIncompatible("TODO")
  ...
  private static class ScheduledListeningDecorator extends ListeningDecorator implements
    ...
    private static final class ListenableScheduledTask<V> extends
        SimpleForwardingListenableFuture<V> implements ListenableScheduledFuture<V> {
   ...
   * .cgi/jsr166/src/test/tck/AbstractExecutorServiceTest.java?revision=1.30 which contained the
   * following notice:
   * 
   * Written by Doug Lea with assistance from members of JCP JSR-166 Expert Group and released to
   * the public domain, as explained at http://creativecommons.org/publicdomain/zero/1.0/ Other
   ...
   *
   * <p>
   * The names are retrieved from the {@code nameSupplier} on the thread that is being renamed right
   * before each task is run. The renaming is best effort, if a {@link SecurityManager} prevents the
   * renaming then it will be skipped but the tasks will still execute.
   *
   *
   ...
   *
   * <p>
   * If, at any step of the process, the given executor is terminated or the calling thread is
   * interrupted, the method calls {@link ExecutorService#shutdownNow()}, cancelling pending tasks
   * and interrupting running tasks.
   *
   * @param service the {@code ExecutorService} to shut down
   * @param timeout the maximum time to wait for the {@code ExecutorService} to terminate
   * @param unit the time unit of the timeout argument
   * @return {@code true} if the pool was terminated successfully, {@code false} if the
   *         {@code ExecutorService} could not terminate <b>or</b> the thread running this method is
   *         interrupted while waiting for the {@code ExecutorService} to terminate
   * @since 17.0
   */
  @Beta
  @GwtIncompatible("concurrency")
  public static boolean shutdownAndAwaitTermination(ExecutorService service, long timeout,
      TimeUnit unit) {
    checkNotNull(unit);
