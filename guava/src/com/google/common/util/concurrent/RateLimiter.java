 ...
 * closely related, e.g. see <a href="http://en.wikipedia.org/wiki/Little's_law">Little's Law</a>).
 *
 * <p>
 ...
 *
 * <p>
 * It is important to note that the number of permits requested <i>never</i> affect the throttling
...
public abstract class RateLimiter {
   ...
   *
   * <p>
   * The returned {@code RateLimiter} ensures that on average no more than {@code permitsPerSecond}
   * are issued during any given second, with sustained requests being smoothly spread over each
   * second. When the incoming request rate exceeds {@code permitsPerSecond} the rate limiter will
   * release one permit every {@code (1.0 / permitsPerSecond)} seconds. When the rate limiter is
   * unused, bursts of up to {@code permitsPerSecond} permits will be allowed, with subsequent
   ...
   *
   * <p>
   * The returned {@code RateLimiter} starts in a "cold" state (i.e. the warmup period will follow),
