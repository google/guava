...
/**
 * <p>
 * A list of listeners, each with an associated {@code Executor}, that guarantees that every
 * {@code Runnable} that is {@linkplain #add added} will be executed after {@link #execute()} is
 * called. Any {@code Runnable} added after the call to {@code execute} is still guaranteed to
 * execute. There is no guarantee, however, that listeners will be executed in the order that they
 * are added.
 *
 * <p>
...
public final class ExecutionList {
   ...
   * some caveats: First, the thread that the listener runs in depends on whether the
   * {@code ExecutionList} has been executed at the time it is added. In particular, listeners may
   * run in the thread that calls {@code add}. Second, the thread that calls {@link #execute} may be
   * an internal implementation thread, such as an RPC network thread, and {@code directExecutor()}
   * listeners may run in this thread. Finally, during the execution of a {@code directExecutor}
   * listener, all other registered but unexecuted listeners are prevented from running, even if
