...
public class CycleDetectingLockFactory {
  ...
  public interface Policy {
     ...
     * <p>
     * Note that the method will be called even upon an invocation of {@code tryLock()}. Although
  ...
  public enum Policies implements Policy {
     ...
     * <p>
     * Note that locks created by a factory with this policy will <em>not</em> participate the cycle
     * detection performed by locks created by other factories.
     */
    DISABLED {
      @Override
      public void handlePotentialDeadlock(PotentialDeadlockException e) {}
   ...
   *
   * <p>
   * As with all locks created by instances of {@code CycleDetectingLockFactory} explicitly ordered
   * locks participate in general cycle detection with all other cycle detecting locks, and a lock's
   * behavior when detecting a cyclic lock acquisition is defined by the {@code Policy} of the
   * factory that created it.
   *
   * <p>
   * Note, however, that although multiple locks can be created for a given Enum value, whether it
   * be through separate factory instances or through multiple calls to the same factory, attempting
   * to acquire multiple locks with the same Enum value (within the same thread) will result in an
   ...
   */
  @Beta
  public static final class WithExplicitOrdering<E extends Enum<E>> extends
  ...
  private static class LockGraphNode {
     ...
     */
    final Map<LockGraphNode, ExampleStackTrace> allowedPriorLocks = new MapMaker().weakKeys()
        .makeMap();
    ...
    void checkAcquiredLock(Policy policy, LockGraphNode acquiredLock) {
      ...
      Preconditions.checkState(this != acquiredLock,
          "Attempted to acquire multiple locks with the same rank " + acquiredLock.getLockName());
      ...
      if (previousDeadlockException != null) {
        ...
        PotentialDeadlockException exception =
            new PotentialDeadlockException(acquiredLock, this,
  ...
  final class CycleDetectingReentrantReadWriteLock extends ReentrantReadWriteLock implements
      CycleDetectingLock {
