 ...
 *
 * <p>
 * Most commonly, {@code ListenableFuture} is used as an input to another derived {@code Future}, as
 * in {@link Futures#allAsList(Iterable) Futures.allAsList}. Many such methods are impossible to
 * implement efficiently without listener support.
 *
 * <p>
 * It is possible to call {@link #addListener addListener} directly, but this is uncommon because
 ...
 *
 * <p>
 * Developers are encouraged to return {@code ListenableFuture} from their methods so that users can
 * take advantages of the utilities built atop the class. The way that they will create
 ...
 * return type. For this case, we provide a more expensive workaround in {@code JdkFutureAdapters}.
 * However, when possible, it is more efficient and reliable to create a {@code ListenableFuture}
 * directly.
 *
 * @author Sven Mawson
...
public interface ListenableFuture<V> extends Future<V> {
   ...
   *
   * <p>
   * There is no guaranteed ordering of execution of listeners, but any listener added through this
   ...
   * {@link MoreExecutors#directExecutor}. For heavier listeners, {@code directExecutor()} carries
   * some caveats. For example, the listener may run on an unpredictable or undesirable thread:
   *
   * <ul>
   * <li>If this {@code Future} is done at the time {@code addListener} is called,
   * {@code addListener} will execute the listener inline.
   * <li>If this {@code Future} is not yet done, {@code addListener} will schedule the listener to
   * be run by the thread that completes this {@code Future}, which may be an internal system thread
   * such as an RPC network thread.
   * </ul>
   *
   * <p>
   * Also note that, regardless of which thread executes the {@code directExecutor()} listener, all
   * other registered but unexecuted listeners are prevented from running during its execution, even
   ...
   * see {@link com.google.common.util.concurrent.Futures}. For a simplified but general listener
   * interface, see {@link com.google.common.util.concurrent.Futures#addCallback addCallback()}.
   *
   * <p>
