...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
  ...
  LocalCache(CacheBuilder<? super K, ? super V> builder, @Nullable CacheLoader<? super K, V> loader) {
    ...
    removalNotificationQueue =
        (removalListener == NullListener.INSTANCE) ? LocalCache
   ...
   * partially collected, but not yet cleaned up - Unset: marked as unset, awaiting cleanup or reuse
   */
  ...
  static class Segment<K, V> extends ReentrantLock {
    ...
    Segment(LocalCache<K, V> map, int initialCapacity, long maxSegmentWeight,
      ...
      writeQueue =
          map.usesWriteQueue() ? new WriteQueue<K, V>() : LocalCache
              .<ReferenceEntry<K, V>>discardingQueue();
            ...
            if (entryValue == null) {
              if (valueReference.isActive()) {
                ...
                ReferenceEntry<K, V> newFirst =
                    removeValueFromChain(first, e, entryKey, hash, valueReference,
      ...
      try {
        ...
        for (ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
          ...
          if (e.getHash() == hash && entryKey != null
            ...
            ++modCount;
            ReferenceEntry<K, V> newFirst =
                removeValueFromChain(first, e, entryKey, hash, valueReference, cause);
            newCount = this.count - 1;
            table.set(index, newFirst);
            this.count = newCount; // write-volatile
            return entryValue;
      ...
      try {
        ...
        for (ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
          if (e == entry) {
            ++modCount;
            ReferenceEntry<K, V> newFirst =
                removeValueFromChain(first, e, e.getKey(), hash, e.getValueReference(),
                    RemovalCause.COLLECTED);
            newCount = this.count - 1;
            table.set(index, newFirst);
            this.count = newCount; // write-volatile
    ...
    boolean removeEntry(ReferenceEntry<K, V> entry, int hash, RemovalCause cause) {
      ...
      for (ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
        if (e == entry) {
          ++modCount;
          ReferenceEntry<K, V> newFirst =
              removeValueFromChain(first, e, e.getKey(), hash, e.getValueReference(), cause);
          newCount = this.count - 1;
          table.set(index, newFirst);
          this.count = newCount; // write-volatile
          return true;
  ...
  static final class WriteQueue<K, V> extends AbstractQueue<ReferenceEntry<K, V>> {
    ...
    public int size() {
      int size = 0;
      for (ReferenceEntry<K, V> e = head.getNextInWriteQueue(); e != head; e =
  ...
  static final class AccessQueue<K, V> extends AbstractQueue<ReferenceEntry<K, V>> {
    ...
    public int size() {
      int size = 0;
      for (ReferenceEntry<K, V> e = head.getNextInAccessQueue(); e != head; e =
