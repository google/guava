...
public final class TypeResolver {
   ...
   * {@code String.class}, then {@code new TypeResolver().where(formal, actual)} will
  ...
  private static void populateTypeMappings(final Map<TypeVariableKey, Type> mappings, Type from,
      final Type to) {
    if (from.equals(to)) {
      return;
    }
    new TypeVisitor() {
      ...
      void visitParameterizedType(ParameterizedType fromParameterizedType) {
        ...
        checkArgument(fromArgs.length == toArgs.length, "%s not compatible with %s",
  ...
  private static class TypeTable {
    ...
    Type resolveInternal(TypeVariable<?> var, TypeTable forDependants) {
      ...
      if (type == null) {
        ...
        return Types.newArtificialTypeVariable(var.getGenericDeclaration(), var.getName(),
            resolvedBounds);
      }
   ...
   *
   * <p>
   * While resolving a type variable from a {var -> type} map, we don't care whether the type
   * variable's bound has been partially resolved. As long as the type variable "identity" matches.
   *
   * <p>
   * On the other hand, if for example we are resolving List<A extends B> to List<A extends String>,
   * we need to compare that <A extends B> is unequal to <A extends String> in order to decide to
