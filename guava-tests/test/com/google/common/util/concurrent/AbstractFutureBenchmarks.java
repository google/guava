...
final class AbstractFutureBenchmarks {
  ...
  private static class NewAbstractFutureFacade<T> extends AbstractFuture<T>
      implements Facade<T> {
    @CanIgnoreReturnValue
    @Override
  ...
  private static final class NewTrustedFutureFacade<T> extends NewAbstractFutureFacade<T>
      implements AbstractFuture.Trusted<T> {
  }

  private static final class NewPassiveTrustedFutureFacade<T> extends NewAbstractFutureFacade<T>
      implements AbstractFuture.Trusted<T> {
    @Override
  ...
  private static final class NewPassiveFutureFacade<T> extends NewAbstractFutureFacade<T> {
    @Override
    protected boolean requiresAfterDoneCallback() {
      return false;
    }
  ...
  private static final class OldTrustedFutureFacade<T> extends OldAbstractFutureFacade<T>
      implements AbstractFuture.Trusted<T> { 
  }
  ...
  enum Impl {
    ...
    NEW_TRUST_PASSIVE {
        @Override
        <T> Facade<T> newFacade() {
            return new NewPassiveTrustedFutureFacade<T>();
        }
    },
    OLD {
      @Override
    ...
    OLD_TRUST {
      @Override
      <T> Facade<T> newFacade() {
        return new OldTrustedFutureFacade<T>();
      }
    };
...
//  abstract static class OldAbstractFuture<V> implements ListenableFuture<V> {}
  ...
  abstract static class OldAbstractFuture<V> extends InternalFutureFailureAccess
 ...
 */
abstract static class TrustedFuture<V> extends OldAbstractFuture<V> implements Trusted<V> {
  @CanIgnoreReturnValue
  @Override
    ...
    try {
      helper =
          new SafeAtomicHelper(
              ...
              newUpdater(OldAbstractFuture.class, Waiter.class, "waiters"),
              newUpdater(OldAbstractFuture.class, Listener.class, "listeners"),
              newUpdater(OldAbstractFuture.class, Object.class, "value"));
...
private static final class SetFuture<V> implements Runnable {
  final OldAbstractFuture<V> owner;
  final ListenableFuture<? extends V> future;

  SetFuture(OldAbstractFuture<V> owner, ListenableFuture<? extends V> future) {
    this.owner = owner;
    this.future = future;
  }
...
public boolean cancel(boolean mayInterruptIfRunning) {
  ...
  if (localValue == null | localValue instanceof SetFuture) {
    ...
    OldAbstractFuture<?> abstractFuture = this;
    while (true) {
      if (ATOMIC_HELPER.casValue(abstractFuture, localValue, valueToSet)) {
        ...
        if (localValue instanceof SetFuture) {
          ...
          if (futureToPropagateTo instanceof Trusted) {
            ...
            OldAbstractFuture<?> trusted = (OldAbstractFuture<?>) futureToPropagateTo;
...
private static Object getFutureValue(ListenableFuture<?> future) {
  if (future instanceof Trusted) {
    ...
    Object v = ((OldAbstractFuture<?>) future).value;
...
private static void complete(OldAbstractFuture<?> future) {
  Listener next = null;
  outer:
  while (true) {
...
private abstract static class AtomicHelper {
  ...
  abstract boolean casWaiters(OldAbstractFuture<?> future, Waiter expect, Waiter update);

  /** Performs a CAS operation on the {@link #listeners} field. */
  abstract boolean casListeners(OldAbstractFuture<?> future, Listener expect, Listener update);

  /** Performs a CAS operation on the {@link #value} field. */
  abstract boolean casValue(OldAbstractFuture<?> future, Object expect, Object update);
}
...
private static final class UnsafeAtomicHelper extends AtomicHelper {
  ...
  /** Performs a CAS operation on the {@link #waiters} field. */
  @Override
  boolean casWaiters(OldAbstractFuture<?> future, Waiter expect, Waiter update) {
    return UNSAFE.compareAndSwapObject(future, WAITERS_OFFSET, expect, update);
  }

  /** Performs a CAS operation on the {@link #listeners} field. */
  @Override
  boolean casListeners(OldAbstractFuture<?> future, Listener expect, Listener update) {
    return UNSAFE.compareAndSwapObject(future, LISTENERS_OFFSET, expect, update);
  }

  /** Performs a CAS operation on the {@link #value} field. */
  @Override
  boolean casValue(OldAbstractFuture<?> future, Object expect, Object update) {
...
private static final class SafeAtomicHelper extends AtomicHelper {
  ...
  final AtomicReferenceFieldUpdater<OldAbstractFuture, Waiter> waitersUpdater;
  final AtomicReferenceFieldUpdater<OldAbstractFuture, Listener> listenersUpdater;
  final AtomicReferenceFieldUpdater<OldAbstractFuture, Object> valueUpdater;

  SafeAtomicHelper(
      AtomicReferenceFieldUpdater<Waiter, Thread> waiterThreadUpdater,
      AtomicReferenceFieldUpdater<Waiter, Waiter> waiterNextUpdater,
      AtomicReferenceFieldUpdater<OldAbstractFuture, Waiter> waitersUpdater,
      AtomicReferenceFieldUpdater<OldAbstractFuture, Listener> listenersUpdater,
      AtomicReferenceFieldUpdater<OldAbstractFuture, Object> valueUpdater) {
