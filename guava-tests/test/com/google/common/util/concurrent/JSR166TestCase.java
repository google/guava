...
/*
 * Source:
 * http://gee.cs.oswego.edu/cgi-bin/viewcvs.cgi/jsr166/src/test/tck/JSR166TestCase.java?revision
 * =1.90 (We have made some trivial local modifications (commented out uncompilable code).)
 */
 ...
 *
 * <li>All delays and timeouts must use one of the constants {@code SHORT_DELAY_MS},
 * {@code SMALL_DELAY_MS}, {@code MEDIUM_DELAY_MS}, {@code LONG_DELAY_MS}. The idea here is that a
 * SHORT is always discriminable from zero time, and always allows enough time for the small amounts
 * of computation (creating a thread, calling a few methods, etc) needed to reach a timeout point.
 * Similarly, a SMALL is always discriminable as larger than SHORT and smaller than MEDIUM. And so
 * on. These constants are set to conservative values, but even so, if there is ever any doubt, they
 * can all be increased in one spot to rerun tests on slower platforms.</li>
 *
 ...
 *
 * <li>As a convenience, the {@code main} of this class (JSR166TestCase) runs all JSR166 unit tests.
 * </li>
 *
 * </ul>
 */
