...
public class FilesSimplifyPathTest extends TestCase {
  ...
  public void testExtensiveNoPrefix() throws IOException {
     ...
     * Run the test. - Pull the relevant lines of output from the test into a testcases file. - Test
     * the output: --- cat testcases | while read L; do X=($L) A=$( cd /b/c && sudo mkdir -p ${X[0]}
     * && cd ${X[0]} && pwd | sed -e 's#^//*#/#' ) B=$( cd /b/c && cd ${X[1]} && pwd ) cmp -s <(echo
     * $A) <(echo $B) || echo "$X[0] -> $A vs. $B" done | tee testoutput - Move that testcases file
     * to the appropriate name under testdata.
     * 
