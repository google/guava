...
public class ConstraintsTest extends TestCase {
  ...
  private static final class TestElementException extends IllegalArgumentException {
  ...
  public void testConstrainedCollectionLegal() {
    ...
    assertThat(collection).containsExactly("foo", "bar", TEST_ELEMENT, "qux", "cat", "dog")
        .inOrder();
    assertThat(constrained).containsExactly("foo", "bar", TEST_ELEMENT, "qux", "cat", "dog")
        .inOrder();
  }
  ...
  public void testConstrainedSortedSetLegal() {
    ...
    assertThat(sortedSet).containsExactly("bar", "cat", "dog", "foo", "qux", TEST_ELEMENT)
        .inOrder();
    assertThat(constrained).containsExactly("bar", "cat", "dog", "foo", "qux", TEST_ELEMENT)
        .inOrder();
  ...
  public void testConstrainedListLegal() {
    ...
    assertThat(list).containsExactly("foo", "cow", "baz", TEST_ELEMENT, "box", "fan", "qux", "cat",
        "dog").inOrder();
    assertThat(constrained).containsExactly("foo", "cow", "baz", TEST_ELEMENT, "box", "fan", "qux",
        "cat", "dog").inOrder();
    ListIterator<String> iterator = constrained.listIterator();
    iterator.next();
    iterator.set("sun");
    constrained.listIterator(2).add("sky");
    assertThat(list).containsExactly("sun", "cow", "sky", "baz", TEST_ELEMENT, "box", "fan", "qux",
        "cat", "dog").inOrder();
    assertThat(constrained).containsExactly("sun", "cow", "sky", "baz", TEST_ELEMENT, "box", "fan",
        "qux", "cat", "dog").inOrder();
  ...
  /**
   * Returns a "nefarious" collection, which permits only one call to iterator(). This verifies that
   * the constrained collection uses a defensive copy instead of potentially checking the elements
   * in one snapshot and adding the elements from another.
   *
  ...
  static <E> Collection<E> onceIterableCollection(final E element) {
    return new AbstractCollection<E>() {
      ...
      public int size() {
        /*
         * We could make the collection empty, but that seems more likely to trigger special cases
         * (so maybe we should test both empty and nonempty...).
         */
        return 1;
      }
