...
public class MapConstraintsTest extends TestCase {
  ...
  public void testConstrainedMapLegal() {
    ...
    assertThat(map.entrySet())
        .containsExactly(Maps.immutableEntry(TEST_KEY, TEST_VALUE), Maps.immutableEntry("foo", 1),
  ...
  public void testConstrainedBiMapLegal() {
    BiMap<String, Integer> map =
        new AbstractBiMap<String, Integer>(Maps.<String, Integer>newLinkedHashMap(),
  ...
  public void testConstrainedMultimapLegal() {
    ...
    assertThat(multimap.entries()).containsExactly(Maps.immutableEntry(TEST_KEY, TEST_VALUE),
        Maps.immutableEntry("foo", 1), Maps.immutableEntry("bar", 2),
        Maps.immutableEntry("baz", 3), Maps.immutableEntry("qux", 4),
        Maps.immutableEntry("zig", 5), Maps.immutableEntry("zag", 6),
        Maps.immutableEntry("bee", 7), Maps.immutableEntry("bim", 8),
        Maps.immutableEntry("bop", 9), Maps.immutableEntry("dig", 10),
  ...
  public void testConstrainedTypePreservingList() {
    ListMultimap<String, Integer> multimap =
        MapConstraints.constrainedListMultimap(LinkedListMultimap.<String, Integer>create(),
            TEST_CONSTRAINT);
    multimap.put("foo", 1);
  ...
  public void testConstrainedTypePreservingRandomAccessList() {
    ListMultimap<String, Integer> multimap =
        MapConstraints.constrainedListMultimap(ArrayListMultimap.<String, Integer>create(),
            TEST_CONSTRAINT);
    multimap.put("foo", 1);
    Map.Entry<String, Collection<Integer>> entry = multimap.asMap().entrySet().iterator().next();
    assertTrue(entry.getValue() instanceof List);
  ...
  public void testConstrainedTypePreservingSet() {
    SetMultimap<String, Integer> multimap =
        MapConstraints.constrainedSetMultimap(LinkedHashMultimap.<String, Integer>create(),
            TEST_CONSTRAINT);
    multimap.put("foo", 1);
    Map.Entry<String, Collection<Integer>> entry = multimap.asMap().entrySet().iterator().next();
  ...
  /**
   * Returns a "nefarious" map, which permits only one call to its views' iterator() methods. This
   * verifies that the constrained map uses a defensive copy instead of potentially checking the
   * elements in one snapshot and adding the elements from another.
   *
  ...
  static <K, V> Map<K, V> onceIterableMap(K key, V value) {
    ...
    return new AbstractMap<K, V>() {
      ...
      public int size() {
        /*
         * We could make the map empty, but that seems more likely to trigger special cases (so
         * maybe we should test both empty and nonempty...).
         */
        return 1;
      }
