...
public class LinkedHashMultimapTest extends TestCase {
  ...
  public void testSerializationOrderingKeysAndEntries() {
    ...
    assertThat(multimap.entries()).containsExactly(mapEntry("b", 2), mapEntry("a", 3),
  ...
  public void testToStringNullExact() {
    ...
    assertEquals("{foo=[3, -1, 2, null, 4, 1], null=[null, 0, -1], bar=[1, 2, null, 3]}",
        multimap.toString());
  }
  ...
  public void testEntriesIteration() {
    ...
    Set<Entry<String, Integer>> set =
        Sets.newLinkedHashSet(asList(Maps.immutableEntry("foo", 2), Maps.immutableEntry("foo", 3),
            Maps.immutableEntry("bar", 4), Maps.immutableEntry("bar", 5),
  ...
  public void testKeySetIteration() {
    new IteratorTester<String>(6, MODIFIABLE, newLinkedHashSet(asList("foo", "bar", "baz", "dog",
        "cat")), IteratorTester.KnownOrder.KNOWN_ORDER) {
      private Multimap<String, Integer> multimap;
