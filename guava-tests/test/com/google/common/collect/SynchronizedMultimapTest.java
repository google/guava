...
public class SynchronizedMultimapTest extends TestCase {
  ...
  private static final class TestMultimap<K, V> extends ForwardingSetMultimap<K, V> implements
      Serializable {
  ...
  public void testSynchronizedListMultimap() {
    ...
    assertThat(multimap.replaceValues("bar", Arrays.asList(6, 5))).containsExactly(1, 2, 3, 1)
        .inOrder();
  ...
  public void testSynchronizedSortedSetMultimap() {
    ...
    assertFalse(multimap.containsKey("foo"));
    assertThat(multimap.replaceValues("bar", Arrays.asList(6, 5))).containsExactly(1, 2, 3)
        .inOrder();
