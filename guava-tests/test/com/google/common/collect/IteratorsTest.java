...
public class IteratorsTest extends TestCase {
  ...
  public void testGetOnlyElement_noDefault_moreThanOneLessThanFiveElements() {
    ...
    try {
      ...
      assertEquals("expected one element but was: <one, two>", expected.getMessage());
    }
  }
  ...
  public void testGetOnlyElement_noDefault_fiveElements() {
    ...
    try {
      ...
      assertEquals("expected one element but was: " + "<one, two, three, four, five>",
          expected.getMessage());
    }
  }
  ...
  public void testGetOnlyElement_withDefault_two() {
    ...
    try {
      ...
      assertEquals("expected one element but was: <foo, bar>", expected.getMessage());
    }
  }
  ...
  public void testTryFind_firstElement() {
    Iterable<String> list = Lists.newArrayList("cool", "pants");
    Iterator<String> iterator = list.iterator();
    assertEquals("cool", Iterators.tryFind(iterator, Predicates.equalTo("cool")).get());
  }

  public void testTryFind_lastElement() {
    Iterable<String> list = Lists.newArrayList("cool", "pants");
    Iterator<String> iterator = list.iterator();
    assertEquals("pants", Iterators.tryFind(iterator, Predicates.equalTo("pants")).get());
  }

  public void testTryFind_alwaysTrue() {
    Iterable<String> list = Lists.newArrayList("cool", "pants");
    Iterator<String> iterator = list.iterator();
    assertEquals("cool", Iterators.tryFind(iterator, Predicates.alwaysTrue()).get());
  }
  ...
  public void testTryFind_alwaysFalse_isPresent() {
    Iterable<String> list = Lists.newArrayList("cool", "pants");
    Iterator<String> iterator = list.iterator();
    assertFalse(Iterators.tryFind(iterator, Predicates.alwaysFalse()).isPresent());
    assertFalse(iterator.hasNext());
  }
  ...
  public void testPaddedPartition_general() {
    new IteratorTester<List<Integer>>(5, IteratorFeature.UNMODIFIABLE, ImmutableList.of(
        asList(1, 2, 3), asList(4, 5, 6), asList(7, null, null)),
        IteratorTester.KnownOrder.KNOWN_ORDER) {
      @Override
      protected Iterator<List<Integer>> newTargetIterator() {
        Iterator<Integer> source = Iterators.forArray(1, 2, 3, 4, 5, 6, 7);
  ...
  public void testSingletonIterator() {
    new IteratorTester<Integer>(3, UNMODIFIABLE, singleton(1),
        IteratorTester.KnownOrder.KNOWN_ORDER) {
      @Override
      protected Iterator<Integer> newTargetIterator() {
