...
public class HashingTest extends TestCase {
  ...
  public void testSipHash24() {
    ...
    assertEquals("Hashing.sipHash24(506097522914230528, 1084818905618843912)", Hashing.sipHash24()
        .toString());
  }
  ...
  public void testConsistentHash_linearCongruentialGeneratorCompatibility() {
    int[] golden100 =
        {0, 55, 62, 8, 45, 59, 86, 97, 82, 59, 73, 37, 17, 56, 86, 21, 90, 37, 38, 83};
  ...
  public void testCombineOrdered() {
    ...
    assertFalse(Hashing.combineOrdered(ImmutableList.of(hash31, hash32)).equals(
  ...
  public void testConcatenatedHashFunction_equals() {
    assertEquals(new ConcatenatedHashFunction(Hashing.md5()),
        new ConcatenatedHashFunction(Hashing.md5()));
    assertEquals(new ConcatenatedHashFunction(Hashing.md5(), Hashing.murmur3_32()),
        new ConcatenatedHashFunction(Hashing.md5(), Hashing.murmur3_32()));
  }

  public void testConcatenatedHashFunction_bits() {
    assertEquals(Hashing.md5().bits(), new ConcatenatedHashFunction(Hashing.md5()).bits());
    assertEquals(Hashing.md5().bits() + Hashing.murmur3_32().bits(), new ConcatenatedHashFunction(
        Hashing.md5(), Hashing.murmur3_32()).bits());
    assertEquals(Hashing.md5().bits() + Hashing.murmur3_32().bits() + Hashing.murmur3_128().bits(),
        new ConcatenatedHashFunction(Hashing.md5(), Hashing.murmur3_32(), Hashing.murmur3_128())
            .bits());
  }
  ...
  private static final ImmutableTable<HashFunction, String, String> KNOWN_HASHES = ImmutableTable
      .<HashFunction, String, String>builder()
      ...
      .put(Hashing.crc32c(), EMPTY_STRING, "00000000").put(Hashing.crc32c(), TQBFJOTLD, "04046222")
      .put(Hashing.crc32c(), TQBFJOTLDP, "b3970019").build();
  ...
  public void testKnownUtf8Hashing() {
    for (Cell<HashFunction, String, String> cell : KNOWN_HASHES.cellSet()) {
      ...
      assertEquals(String.format(Locale.ROOT, "Known hash for hash(%s, UTF_8) failed", input),
          expected, func.hashString(input, Charsets.UTF_8).toString());
    }
  }

  public void testNullPointers() {
    NullPointerTester tester =
        new NullPointerTester().setDefault(HashCode.class, HashCode.fromLong(0));
   ...
   * different parameters can be equal. That fact is a problem for
   * {@code testSeededHashFunctionEquals}.
   */
  ...
  static void assertSeededHashFunctionEquals(Class<?> clazz) throws Exception {
    ...
    for (Method method : clazz.getDeclaredMethods()) {
      if (method.getReturnType().equals(HashFunction.class) // must return HashFunction
        ...
        new EqualsTester().addEqualityGroup(hashFunction1a, hashFunction1b)
            .addEqualityGroup(hashFunction2).testEquals();
