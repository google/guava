...
public class MapMakerComparisonBenchmark {
  ...
  private final LoadingCache<Object, Object> loadingCache = CacheBuilder.newBuilder().recordStats()
      .build(CacheLoader.from(IDENTITY));
  private final LoadingCache<Object, Object> loadingCacheNoStats = CacheBuilder.newBuilder().build(
      CacheLoader.from(IDENTITY));
  ...
  @Benchmark
  void computingMapMaker(int rep) {
    for (int i = 0; i < rep; i++) {
  ...
  @Benchmark
  void loadingCacheBuilder_stats(int rep) {
    for (int i = 0; i < rep; i++) {
  ...
  @Benchmark
  void loadingCacheBuilder(int rep) {
    for (int i = 0; i < rep; i++) {
